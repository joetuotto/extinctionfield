...
BioCap = sum(w_i * marker_i) over the seven positive markers
         + |w_CORT| * (1 - CORT)

Cortisol enters inverted because higher cortisol degrades capacity (its
weight is stored with a negative sign). The absolute weights sum to 1.0,
so BioCap spans [0, 1] and 1.0 is the pre-industrial optimum.
...
def compute_biocap(biomarker_dict: dict[str, float]) -> float:
    ...
    -------
    float
        Weighted BioCap score in [0, 1]: 1.0 when every positive marker is
        at 1.0 and cortisol at 0.0; 0.0 when every positive marker is at
        0.0 and cortisol at 1.0. Markers with a negative weight (cortisol)
        contribute |w| * (1 - value), so the absolute weights sum to 1.0.
    ...
    for k in REQUIRED_MARKERS:
        w = BIOMARKER_WEIGHTS[k]
        if w < 0:
            raw += -w * (1.0 - biomarker_dict[k])
        else:
            raw += w * biomarker_dict[k]
