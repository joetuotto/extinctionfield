...
@dataclass(frozen=True)
class RetardedResponseResult:
    """Auditable discrete contraction of a lagged, state-conditioned kernel.

    The lag contributions are retained so a fit cannot hide which portion of
    the response came from the current perturbation and which came from its
    declared history.  The caller still supplies every kernel value; BERM does
    not infer a tissue coefficient from FieldState observations.
    """

    lag_contributions: tuple[float, ...]
    total: float
...
def contract_retarded_response(
    response_kernel_history: ArrayLike,
    delta_metric_history: ArrayLike,
    *,
    lag_weights: ArrayLike | None = None,
) -> RetardedResponseResult:
    """Contract a discrete retarded kernel with a metric-perturbation history.

    The leading axis indexes matched lags and the remaining axes are the
    tensor indices of ``Xi_i`` and ``delta_g``.  This is the discrete form of

    ``u_i(t) = integral K_i^{mu nu}(tau; S_i(t-tau))
                        delta_g_mu_nu(t-tau) d tau``.

    Orientation, coherence, waveform, temperature trajectory and biological
    state belong to the caller-declared kernel/context.  This function only
    performs the explicit contraction and therefore does not calibrate the
    open BERM L2 bridge.
    """

    kernels = np.asarray(response_kernel_history, dtype=float)
    perturbations = np.asarray(delta_metric_history, dtype=float)
    if kernels.shape != perturbations.shape or kernels.ndim < 3:
        raise ValueError(
    ...
    if lag_weights is None:
        weights = np.ones(kernels.shape[0], dtype=float)
    else:
        weights = _finite_vector("lag_weights", lag_weights)
        if weights.shape != (kernels.shape[0],):
            raise ValueError("lag_weights must align with the leading lag axis")

    tensor_axes = tuple(range(1, kernels.ndim))
    contributions = weights * np.sum(kernels * perturbations, axis=tensor_axes)
    return RetardedResponseResult(
        lag_contributions=tuple(float(value) for value in contributions),
