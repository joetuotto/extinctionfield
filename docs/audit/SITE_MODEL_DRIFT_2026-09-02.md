...
**Avoinna (seuraava erä):** sivilisaatiosivujen D1–D3-avaimet (patopoliteia vaiheet/siirtymät/herkkyys/tutka/migraatio/L1–L3/α–β-esimerkit, patokratia :83/:125/:109, pathopolites :25/:61/:99, patokinesis kaavat + s6-taulukot + maaluvut, patopolis TFR-luvut ja n:t, pathopege RCT-nimet) — odottaa rinnakkaisen session "done"-ilmoitusta; `political_biology.py` docstringit ja JSON `berm_mechanism` (D3); `v16.py:1710–1728` polkukirjaimet ja rekisterin `pathway`-tagit (D5, tiedostot rinnakkaisen session hallussa); `causalChainData.ts` polkukorttien nimeäminen (C melatoniini → B:n melatoniinihaara, E BBB → C, F Vmem ilman kirjainta); 92 viitemerkintää; evolution-sivun 4 falsifikaatiokriteeriä; FI-käännökset; osan 7 integraatiot; ennustesivun laskureiden johtaminen datasta; auditoimattomat alueet (osa 10).
