...
const DISEASES: Disease[] = [
  {
    ...
    fda_mechanism_match: { en: "rTMS modulates the same neuromodulatory level that EMF disrupts", fi: "rTMS säätelee samaa neuromodulatorista tasoa jonka EMF häiritsee", ja: "rTMSはEMFが障害する同じ神経調節レベルを調節する", fr: "La rTMS module le même niveau neuromodulateur que l'EMF perturbe", ko: "rTMS는 EMF가 교란하는 동일한 신경조절 수준을 조절함" },
    ...
    covid_channel: "RF ↑",
    covid_evidence: { en: "GBD: sleep disorder prevalence +81% peak in 2020 vs 1990 baseline", fi: "GBD: unihäiriöesiintyvyys +81 % huippu 2020 vs. 1990-baseline", ja: "GBD：睡眠障害有病率 2020年ピーク時1990年基準比+81%", fr: "GBD : prévalence des troubles du sommeil +81 % au pic 2020 vs référence 1990", ko: "GBD: 수면 장애 유병률 2020년 정점 시 1990년 기준 대비 +81%" },
  ...
  },
  {
    ...
    fda_mechanism_match: { en: "VNS modulates vagal reflex controlling neuroinflammation. EMF disrupts the same reflex (anti-VNS hypothesis).", fi: "VNS säätelee vagaalista refleksiä joka säätelee neuroinflammatorista tilaa. EMF häiritsee samaa refleksiä (anti-VNS).", ja: "VNSは神経炎症を制御する迷走神経反射を調節する。EMFは同じ反射を障害する（逆VNS仮説）。", fr: "Le VNS module le réflexe vagal contrôlant la neuroinflammation. L'EMF perturbe le même réflexe (hypothèse anti-VNS).", ko: "VNS는 신경염증을 제어하는 미주신경 반사를 조절함. EMF는 동일 반사를 교란(역-VNS 가설)." },
  ...
  },
  {
    ...
    fda_mechanism_match: { en: "PEMF activates adenosine receptors and modulates ion channels therapeutically. Environmental ELF disrupts the same channels chronically.", fi: "PEMF aktivoi adenosiinireseptoreita ja säätelee ionikanavia terapeuttisesti. Ympäristö-ELF häiritsee samoja kanavia kroonisesti.", ja: "PEMFはアデノシン受容体を活性化しイオンチャネルを治療的に調節する。環境ELFは同じチャネルを慢性的に障害する。", fr: "Le PEMF active les récepteurs d'adénosine et module les canaux ioniques de manière thérapeutique. L'ELF environnemental perturbe chroniquement les mêmes canaux.", ko: "PEMF는 아데노신 수용체를 활성화하고 이온 채널을 치료적으로 조절함. 환경 ELF는 동일 채널을 만성적으로 교란." },
    ...
    covid_evidence: { en: "GBD: T2D prevalence 4.7% → 10.1% (2020), +115% from baseline", fi: "GBD: T2D-esiintyvyys 4,7 % → 10,1 % (2020), +115 % perusviivasta", ja: "GBD：T2D有病率 4.7% → 10.1%（2020年）、基準比+115%", fr: "GBD : prévalence DT2 4,7 % → 10,1 % (2020), +115 % par rapport à la référence", ko: "GBD: T2D 유병률 4.7% → 10.1%(2020), 기준 대비 +115%" },
