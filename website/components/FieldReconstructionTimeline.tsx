...
export function FieldReconstructionTimeline({countryId,locale,from,to,year,onYearChange,compact=false,onExpand}:Props) {
  ...
  const family=fieldReconstruction.families.find(f=>f.id===(selected??tracks[0]?.familyId));
  const cell=family?getReconstructionCell(countryId,family.id,year):null;
  const selectedTrack=all.find(track=>track.familyId===family?.id);
  ...
  return <section ref={container}>
    ...
    <div className={styles.fieldRows}>{tracks.map((track,index)=>{
      ...
      return <div key={f.id} className={styles.fieldRow}>
        <div className={styles.fieldHeading}><button type="button" onClick={()=>{setSelected(f.id);if(compact)onExpand?.();}} aria-expanded={family?.id===f.id&&!compact}>{t(f.label,locale)}</button><span>{year} · {t(reconstructionStageLabels[atYear.stage],locale)}</span></div>
    ...
    {compact ? <button className={styles.textButton} onClick={onExpand}>{fi?"Avaa kaikki lähteet, kenttämuodot ja historiaperusteet →":"Open all sources, field forms and historical evidence →"}</button> : <>
      {family&&cell&&<section className={styles.morphology}><h3>{t(family.label,locale)} · {year}</h3><p className={styles.note}>{t(cell.note,locale)}</p>
        <dl>{(["carrier","waveform","intermittence","geography"] as const).map((key,i)=><div key={key} style={{display:"contents"}}><dt>{(fi?["Taajuudet","Aaltomuoto","Käyttörytmi","Paikallinen muoto"]:["Frequencies","Waveform","Duty cycle","Spatial form"])[i]}</dt><dd>{t(family.morphology[key],locale)}</dd></div>)}</dl>
        {cell.phase && <p className={styles.note}>{fi?"Jakson peruste":"Interval basis"}: {cell.phase.basis==="documented"?(fi?"dokumentoitu tapahtuma":"documented event"):(fi?"historiallinen rekonstruktio":"historical reconstruction")} · {cell.phase.startYear}–{cell.phase.endYear}</p>}
        {cell.anchors.map(a=><p key={a.id} className={styles.note}><strong>{a.startYear}{a.endYear?`–${a.endYear}`:""} · {t(a.title,locale)}</strong><br/>{t(a.scope,locale)}</p>)}
        {getReconstructionSources(cell.sourceRefs).map(s=><p className={styles.note} key={s.id}><a href={s.url} target="_blank" rel="noreferrer">{s.title} ↗</a></p>)}
        <p className={styles.note}>{t(selectedTrack?.gap??cell.note,locale)}</p>
      </section>}
