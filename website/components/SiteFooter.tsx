...
const COPY: Record<string, {
  ...
  en: {
    ...
    summary: (n: number) => `Three-channel framework (ELF · IF · RF). ${n} registered sources.`,
    ...
    summary: (n: number) => `Kolmikanavakehys (ELF · IF · RF). ${n} rekisteröityä lähdettä.`,
  ...
  ja: {
    ...
    summary: (n: number) => `3チャネルフレームワーク（ELF · IF · RF）。${n}件の登録済み資料。`,
  ...
  fr: {
    ...
    summary: (n: number) => `Cadre à trois canaux (ELF · IF · RF). ${n} sources enregistrées.`,
    license: "Code : MIT · Docs : CC BY-4.0",
  ...
  ko: {
    ...
    summary: (n: number) => `3채널 프레임워크 (ELF · IF · RF). ${n}개의 등록 출처.`,
