...
const COPY = {
  en: {
    ...
    curveIntro: "The outcome and its proxy move together. Reveal the proposed EMF branch, then switch the causal case: the same curves can have different explanations.",
    caseLabel: "Causal case",
    cases: ["Shared change", "Intermediate step"],
    reveal: "Reveal the proposed EMF branch",
    hide: "Hide the proposed EMF branch",
    outcome: "Outcome",
    proxies: ["Proxy: technology use", "Proxy: sleep or appetite"],
    field: "Proposed EMF branch",
    ...
    hiddenStatus: "The outcome and proxy are visible. The proposed field branch is hidden.",
    shownStatus: "The proposed field branch is now visible alongside the unchanged outcome and proxy.",
    ...
    commonSummary: "Technological development changes the proxy and the field in parallel. BERM proposes a conditional field → reception → outcome path. The proxy can therefore accompany the outcome without identifying the field contribution.",
    mediatorSummary: "BERM proposes a conditional field → reception → sleep or appetite → outcome path. The measured intermediate step can be a real cause while leaving its own origin outside the analysis.",
    curveCaption: "The curve values are chosen for explanation, not taken from historical data or fitted to an endpoint. Their resemblance does not establish a field effect. The two cases separate a correlated proxy from a causal intermediate step; the proposed biological field link remains conditional.",
    ...
    separateSummary: "The proximate explanations can remain useful. Displaying them separately does not decide whether they share an earlier contribution.",
    connectedSummary: "The same defined receiving and signalling mechanisms can be reused across levels. Each proposed connection still needs its own biological interpretation and calibration.",
    ...
    curveIntro: "Seuraus ja sitä kuvaava proksi muuttuvat rinnakkain. Paljasta oletettu EMF-haara ja vaihda sitten kausaalista tapausta: samat käyrät voivat saada erilaisen selityksen.",
    caseLabel: "Kausaalinen tapaus",
    cases: ["Yhteinen muutos", "Välivaihe"],
    reveal: "Paljasta oletettu EMF-haara",
    hide: "Piilota oletettu EMF-haara",
    outcome: "Seuraus",
    proxies: ["Proksi: teknologian käyttö", "Proksi: uni tai ruokahalu"],
    field: "Oletettu EMF-haara",
    ...
    hiddenStatus: "Seuraus ja proksi näkyvät. Oletettu kenttähaara on piilossa.",
    shownStatus: "Oletettu kenttähaara näkyy nyt muuttumattomien seurauksen ja proksin rinnalla.",
    chainTitle: "Käyrien taustalle ehdotettu syyrakenne",
    ...
    commonSummary: "Teknologinen kehitys muuttaa proksia ja kenttää rinnakkain. BERM ehdottaa ehdollista reittiä kenttä → vastaanotto → seuraus. Proksi voi siten seurata lopputulosta tunnistamatta kentän osuutta.",
    mediatorSummary: "BERM ehdottaa ehdollista reittiä kenttä → vastaanotto → uni tai ruokahalu → seuraus. Mitattu välivaihe voi olla todellinen syy, vaikka sen oma alkuperä jää tarkastelun ulkopuolelle.",
    curveCaption: "Käyrien arvot on valittu havainnollistamista varten; ne eivät ole historiallista aineistoa tai päätepisteeseen sovitettuja tuloksia. Käyrien samankaltaisuus ei osoita kenttävaikutusta. Tapaukset erottavat korreloivan proksin kausaalisesta välivaiheesta; ehdotettu kentän biologinen liitos pysyy ehdollisena.",
    ...
    separateSummary: "Lähiselitykset voivat säilyä hyödyllisinä. Niiden esittäminen erillään ei ratkaise, onko taustalla yhteinen aiempi tekijä.",
    connectedSummary: "Samoja määriteltyjä vastaanoton ja signaloinnin osamekanismeja voidaan käyttää eri tasoilla. Jokainen ehdotettu liitos tarvitsee silti oman biologisen tulkintansa ja kalibrointinsa.",
