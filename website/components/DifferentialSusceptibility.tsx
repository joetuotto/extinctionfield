...
const COPY = {
  en: {
    ...
    p3: "Tämä periaate ulottuu mehiläisten ulkopuolelle. Punkkipopulaatiot (Ixodes, Dermacentor) laajenevat kautta Euroopan ja Pohjois-Amerikan. Punkit käyttävät sähköstaattista isäntäkontaktia ([[ref:england_2023_ticks|England 2023, Current Biology]]) — mekanismia joka voi tehostua muuttuneessa FieldStatessa. Samanaikaisesti niiden saalistajat (hyönteissyöjälinnut, lepakot) vähenevät osin EMF:n magneettikompassin häirinnän vuoksi ([[ref:lindecke2026|Lindecke 2026, Science]]; [[ref:csli_pecbms_27|PECBMS-lintuindeksit]]). Ekologinen kaskadi yhdistyy: vähemmän saalistajia + tehostunut loiskontakti = vektorivälitteisten tautien laajeneminen.",
  ...
  ja: {
    ...
    p3: "この原理はミツバチを超えて適用される。ダニ個体群（Ixodes、Dermacentor）はヨーロッパと北米全域で拡大している。ダニは静電気宿主接触を利用する（[[ref:england_2023_ticks|England 2023, Current Biology]]）— 変化したFieldStateで強化される可能性のあるメカニズム。同時に、その捕食者（食虫鳥、コウモリ）はEMFによる磁気ナビゲーション障害のため減少している（[[ref:lindecke2026|Lindecke 2026, Science]]; [[ref:csli_pecbms_27|PECBMSの鳥類指数]]）。生態学的カスケードは複合する：捕食者の減少 + 寄生虫接触の強化 = 媒介性疾病の拡大。",
  ...
  fr: {
    ...
    p3: "Ce principe s'étend au-delà des abeilles. Les populations de tiques (Ixodes, Dermacentor) s'étendent à travers l'Europe et l'Amérique du Nord. Les tiques utilisent le contact électrostatique avec l'hôte ([[ref:england_2023_ticks|England 2023, Current Biology]]) — un mécanisme qui pourrait être renforcé dans le FieldState modifié. Simultanément, leurs prédateurs (oiseaux insectivores, chauves-souris) déclinent en partie à cause de la perturbation EMF de la navigation magnétique ([[ref:lindecke2026|Lindecke 2026, Science]] ; [[ref:csli_pecbms_27|indices PECBMS]]). La cascade écologique se compose : moins de prédateurs + contact parasitaire renforcé = expansion des maladies vectorielles.",
  ...
  ko: {
    ...
    p3: "이 원리는 꿀벌을 넘어 확장된다. 진드기 개체군(Ixodes, Dermacentor)이 유럽과 북미 전역에서 확대되고 있다. 진드기는 정전기 숙주 접촉을 사용한다([[ref:england_2023_ticks|England 2023, Current Biology]]) — 변화된 FieldState에서 강화될 수 있는 메커니즘. 동시에 그 포식자(식충조류, 박쥐)는 부분적으로 EMF에 의한 자기 항법 교란으로 감소하고 있다([[ref:lindecke2026|Lindecke 2026, Science]]; [[ref:csli_pecbms_27|PECBMS 조류 지수]]). 생태학적 연쇄가 복합된다: 포식자 감소 + 기생충 접촉 강화 = 매개체 전파 질병 확대.",
