...
const RETRODICTIONS: Retrodiction[] = [
  ...
  },
  {
    ...
    explanation: {
      en: "BERM candidate routes include systemic gamete state and local IVF-laboratory conditions. CatSper is essential for sperm navigation, but a direct environmental EMF → human CatSper step has not been established. TTFields and melatonin studies provide separate component analogies; a matched IVF exposure, organ-transfer and mediator protocol is still required.",
      fi: "BERM:n ehdokasreitteihin kuuluvat systeeminen sukusolutila ja paikalliset IVF-laboratorio-olosuhteet. CatSper on välttämätön siittiön suunnistukselle, mutta suoraa ympäristö-EMF → ihmisen CatSper -askelta ei ole osoitettu. TTFields- ja melatoniinitutkimukset ovat erillisiä komponenttianalogioita; sovitettu IVF-altistus-, elinsiirto- ja välittäjäprotokolla puuttuu.",
      ja: "BERMは全身性配偶子状態と局所IVF環境を候補経路とする。CatSperは必須だが、環境EMFからヒトCatSperへの直接段階は未確立である。TTFieldsとメラトニン研究は別々の構成要素類推であり、対応したIVFプロトコルが必要である。",
      fr: "BERM propose l'état systémique des gamètes et les conditions locales du laboratoire FIV comme voies candidates. CatSper est essentiel, mais l'étape directe EMF environnemental → CatSper humain n'est pas établie. TTFields et la mélatonine sont des analogies composantes séparées; un protocole FIV apparié reste nécessaire.",
      ko: "BERM 후보 경로에는 전신 배우자 상태와 국소 IVF 실험실 조건이 포함된다. CatSper는 필수이지만 환경 EMF → 인간 CatSper 직접 단계는 확립되지 않았다. TTFields와 멜라토닌 연구는 별도의 구성요소 유추이며 일치된 IVF 프로토콜이 필요하다.",
    },
    prediction: {
...
const COPY = {
  en: {
    ...
    lead: "Six biological outcomes provide retrospective tests of the twelve-layer modulome. Their proposed convergence is a shared-cause hypothesis: each outcome needs an explicit exposure measure, comparison group and competing explanations. The cards distinguish observations, proposed mechanisms and testable predictions.",
    data: "Data",
    modulomeExplanation: "Modulome explanation",
    testable: "Testable prediction",
    source: "Source",
    status: "Status",
    caveat: "These outcomes are not established as statistically independent. Shared data, selection and confounding must be assessed before combining evidence. No joint probability is assigned here, and the proposed EMF pathway requires direct testing.",
  },
  fi: {
    title: "Retrodiktiot: mitä moduloomi selittää",
    lead: "Kuusi biologista vastetta tarjoaa mahdollisuuden testata kaksitoistakerroksista moduloomia menneillä aineistoilla. Niiden ehdotettu yhtenevyys on yhteisen syyn hypoteesi: jokainen vaste tarvitsee määritellyn altistusmittarin, vertailuryhmän ja kilpailevien selitysten arvioinnin. Kortit erottavat havainnot, ehdotetut mekanismit ja testattavat ennusteet.",
    data: "Data",
    modulomeExplanation: "Moduloomiselitys",
    testable: "Testattava ennuste",
    source: "Lähde",
    status: "Tila",
    caveat: "Näiden vasteiden tilastollista riippumattomuutta ei ole osoitettu. Yhteiset aineistot, valikoituminen ja sekoittavat tekijät on arvioitava ennen näytön yhdistämistä. Tässä ei anneta yhteistä todennäköisyyttä, ja ehdotettu EMF-reitti tarvitsee suoran testin.",
  },
  ja: {
    title: "遡及予測：モジュロームが説明すること",
    lead: "6つの生物学的アウトカムは、12層のモジュロームを過去のデータで検証する機会を提供します。提案された収束は共通原因の仮説です。各アウトカムには明確な曝露指標、比較群、競合する説明の評価が必要です。カードでは観察、提案された機構、検証可能な予測を区別します。",
    data: "データ",
    modulomeExplanation: "モジュロームの説明",
    testable: "検証可能な予測",
    source: "出典",
    status: "ステータス",
    caveat: "これらのアウトカムの統計的独立性は確立していません。証拠を統合する前に、共有データ、選択、交絡を評価する必要があります。ここでは共同確率を割り当てず、提案されたEMF経路には直接的な検証が必要です。",
  },
  fr: {
    title: "Rétrodictions : ce que le modulome explique",
    lead: "Six résultats biologiques permettent de tester rétrospectivement le modulome à douze couches. Leur convergence proposée constitue une hypothèse de cause commune : chaque résultat nécessite une mesure explicite de l’exposition, un groupe de comparaison et l’évaluation des explications concurrentes. Les fiches distinguent observations, mécanismes proposés et prédictions testables.",
    data: "Données",
    modulomeExplanation: "Explication par le modulome",
    testable: "Prédiction testable",
    source: "Source",
    status: "Statut",
    caveat: "L’indépendance statistique de ces résultats n’est pas établie. Les données communes, la sélection et les facteurs de confusion doivent être évalués avant de combiner les preuves. Aucune probabilité conjointe n’est attribuée ici, et la voie EMF proposée nécessite un test direct.",
  },
  ko: {
    title: "소급 예측: 모듈롬이 설명하는 것",
    lead: "여섯 생물학적 결과는 과거 자료로 12층 모듈롬을 검증할 기회를 제공합니다. 제안된 수렴은 공통 원인 가설입니다. 각 결과에는 명확한 노출 측정, 비교 집단과 경쟁 설명의 평가가 필요합니다. 카드는 관찰, 제안된 기전과 검증 가능한 예측을 구분합니다.",
    data: "데이터",
    modulomeExplanation: "모듈롬 설명",
    testable: "검증 가능한 예측",
    source: "출처",
    status: "상태",
    caveat: "이 결과들의 통계적 독립성은 확립되지 않았습니다. 근거를 결합하기 전에 공유 자료, 선택과 교란을 평가해야 합니다. 여기서는 결합 확률을 부여하지 않으며, 제안된 EMF 경로는 직접 검증이 필요합니다.",
  },
} as const;
