...
export const RELATION_LABELS: Record<RelationType, BilingualText> = {
  ...
  bridge: { en: "Proposed coupling · L2 open", fi: "Kytkentäehdotus · L2 avoin" },
...
const canonicalDescriptions: Record<string, [number, string, string, string]> = {
  ...
  berm_l2_bridge: [-2, "[OPEN · L2] A specified response operator must connect δg, the chosen Weyl/GME structure or holonomy to a measurable receiver variable. Units, gauge, observer, transfer and endpoint calibration remain explicit requirements. Downstream component evidence cannot supply this identification by itself.", "[AVOIN · L2] Määritellyn vasteoperaattorin on yhdistettävä δg, valittu Weyl/GME-rakenne tai holonomia mitattavaan vastaanotinmuuttujaan. Yksiköt, gauge, havaitsija, siirto ja päätepistekalibrointi ovat eksplisiittisiä vaatimuksia. Alavirran komponenttinäyttö ei yksin ratkaise tätä identifikaatiota.", "/model"],
  vmem_mtor: [2, "Imported membrane-potential, calcium, AMPK and mTOR biology forms a conditional gate for reserve activation, growth and repair. The geometry-to-membrane coupling remains open.", "Tuotu kalvopotentiaali-, kalsium-, AMPK- ja mTOR-biologia muodostaa ehdollisen portin reservin aktivaatiolle, kasvulle ja korjaukselle. Geometriasta kalvoon johtava kytkentä on avoin.", "/evidence/reproductive-arc"],
  ...
  male_steroidogenesis: [3, "Leydig-cell steroidogenic support combines HPG, melatonin/redox, microbiome and target-readiness states before sperm output.", "Leydigin solujen steroidogeeninen tuki yhdistää HPG:n, melatoniini/redoxin, mikrobiomin ja kohdekudoksen valmiuden ennen siittiötuotantoa.", "/modulome/testes"],
  ...
  demand_opportunity: [5, "Partner formation, family demand and opportunity enter the demographic calculation separately from biological couple capacity. Biological and social feedbacks must be represented through explicit intermediates.", "Parinmuodostus, perhetoive ja mahdollisuudet tulevat demografiseen laskentaan erillään parin biologisesta kapasiteetista. Biologiset ja sosiaaliset palautteet kuvataan eksplisiittisten välivaiheiden avulla.", "/model/biological-coordination"],
...
const canonicalExistingDescriptions: Record<string, BilingualText> = {
  mech_vgcc_ros: { en: "Imported Vmem/VGCC–Ca²⁺–mitochondrial/redox biology. Receptor state and baseline calcium matter; ROS has a functional window. RyR release and SERCA reuptake are coupled parts of an ER–cytosol–mitochondrial cycle, not independent exposure pathways. The geometry-to-observable coupling is open.", fi: "Tuotu Vmem/VGCC–Ca²⁺–mitokondrio/redox-biologia. Vastaanotintila ja lähtökalsium vaikuttavat; ROS:lla on toiminnallinen ikkuna. RyR-vapautuminen ja SERCA-takaisinotto ovat ER–sytosoli–mitokondriokierron kytkeytyneitä osia, eivät riippumattomia altistusreittejä. Geometriasta havaittavaan johtava kytkentä on avoin." },
  mech_cry_melatonin: { en: "Imported radical-pair, cryptochrome, clock and redox mechanisms must be specified by cryptochrome subtype, species, flavin state and light history. Photochemical response does not by itself establish magnetic modulation. A universal coefficient for all eukaryotes is not assumed; the L2 coupling remains open.", fi: "Tuodut radikaalipari-, kryptokromi-, kello- ja redox-mekanismit määritellään kryptokromialatyypin, lajin, flaviinitilan ja valohistorian mukaan. Fotokemiallinen vaste ei yksin osoita magneettista modulaatiota. Kaikille eukaryooteille yhteistä kerrointa ei oleteta; L2-kytkentä on avoin." },
...
const calibrationCopy: Record<string, BilingualText> = {
  ...
  requires_external_measurement: { en: "Measured separately from biological couple capacity.", fi: "Mitataan erillään parin biologisesta kapasiteetista." },
...
const cardNodes: CausalMapNode[] = MECHANISM_CARDS.map(c => ({
  ...
  provenance: { en: "Imported component mechanism; illustrative model coefficients are not calibrated; the L2 geometry coupling is open.", fi: "Tuotu komponenttimekanismi; havainnollistavia mallikertoimia ei ole kalibroitu; L2-geometriakytkentä on avoin." },
...
for (const e of graph.edges) CORE_EDGES.push({
  ...
  relation: e.kind === "inference_input" ? "inference" : e.kind === "proposed_bridge" ? "bridge" : "causal", sourcePaths: [sourceRegistry],
});
