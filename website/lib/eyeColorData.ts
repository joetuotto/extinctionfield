...
export const EVIDENCE_CARDS: EvidenceCard[] = [
  {
    ...
    berm_relevance_en: "Direct evidence that vitamin B2 availability controls CRY protein levels and circadian clock function. Establishes the nutritional prerequisite for BERM pathway B.",
    berm_relevance_fi: "Suora näyttö siitä, että B2-vitamiinin saatavuus kontrolloi CRY-proteiinien tasoja ja sirkadiaanisen kellon toimintaa. Vahvistaa BERM:n polku B:n ravitsemuksellisen edellytyksen.",
    berm_relevance_ja: "ビタミンB2の利用可能性がCRYタンパク質レベルと概日時計機能を制御するという直接的証拠。BERMパスウェイBの栄養的前提条件を確立する。",
    berm_relevance_fr: "Preuve directe que la disponibilité en vitamine B2 contrôle les niveaux de protéine CRY et la fonction de l'horloge circadienne. Établit le prérequis nutritionnel pour la voie B du BERM.",
    berm_relevance_ko: "비타민 B2 이용 가능성이 CRY 단백질 수준과 개일시계 기능을 제어한다는 직접적 증거. BERM 경로 B의 영양적 전제조건을 확립한다.",
  },
  {
    ...
    authors: "Iversen JLY, Tai YK, Franco-Obregón A et al.",
    year: 2025,
    journal: "Cells",
    ...
    berm_relevance_en: "Smoking gun for the nutrition-magnetoreception link. Also reveals that BERM pathways A (VGIC/TRPC1) and B (CRY/RPM) are physically coupled through a CRY2-TRPC1 complex — they are not independent.",
    berm_relevance_fi: "Suora todiste ravitsemuksen ja magnetoreseption yhteydestä. Paljastaa myös, että BERM:n polut A (VGIC/TRPC1) ja B (CRY/RPM) ovat fyysisesti kytketyt CRY2-TRPC1-kompleksin kautta — ne eivät ole riippumattomia.",
    berm_relevance_ja: "栄養と磁気受容の関連を示す決定的証拠。また、BERMパスウェイA（VGIC/TRPC1）とB（CRY/RPM）がCRY2-TRPC1複合体を通じて物理的に結合していることを明らかにした――両者は独立ではない。",
    berm_relevance_fr: "Preuve décisive du lien nutrition-magnétoréception. Révèle également que les voies BERM A (VGIC/TRPC1) et B (CRY/RPM) sont physiquement couplées via un complexe CRY2-TRPC1 — elles ne sont pas indépendantes.",
    berm_relevance_ko: "영양-자기수용 연결의 결정적 증거. 또한 BERM 경로 A(VGIC/TRPC1)와 B(CRY/RPM)가 CRY2-TRPC1 복합체를 통해 물리적으로 결합되어 있음을 밝혔다 — 이들은 독립적이지 않다.",
  },
  {
    id: "higuchi2007-eye-melatonin",
    referenceId: "higuchi2007",
    title_en: "Eye color influences melatonin suppression by light",
    title_fi: "Silmien väri vaikuttaa valon aiheuttamaan melatoniinivaimennukseen",
    ...
    finding_fi: "Sinisilmäisten kaukaasialaisten melatoniinin suppressio oli 89 % verrattuna tummasilmäisten aasialaisten 73 %:iin identtisessä 1000 luksin altistuksessa 2 tunnin ajan. Ero viittaa siihen, että iiriksen pigmentaatio säätelee suoraan ei-visuaalista valoreittiä sirkadiaaniseen kelloon.",
    ...
    berm_relevance_fi: "Kvantifioi iiris-CRY-melatoniini-polun. Siniset silmät tuottavat 22 % voimakkaamman melatoniinin vaimentumisen — suora mittaus polku B:n vahvistuneesta aktivaatiosta.",
    berm_relevance_ja: "虹彩-CRY-メラトニン経路を定量化する。青い目は22%強いメラトニン抑制を生じる――パスウェイB活性化増強の直接的測定値。",
    berm_relevance_fr: "Quantifie la voie iris-CRY-mélatonine. Les yeux bleus produisent une suppression de la mélatonine 22 % plus forte — une mesure directe de l'activation renforcée de la voie B.",
    berm_relevance_ko: "홍채-CRY-멜라토닌 경로를 정량화한다. 파란 눈은 22% 더 강한 멜라토닌 억제를 생성한다 — 경로 B 활성화 증강의 직접적 측정치.",
  },
  {
  ...
  },
  {
    ...
    berm_relevance_en: "CRITICAL for BERM pathway B: Identifies a SECOND CRY system in the human retina. CRY1 in blue cones (sensory/phototransductive) is distinct from CRY2 in ganglion cells (circadian). This is the system most directly affected by iris pigmentation — blue eyes transmit ~100x more light specifically to blue cones where CRY1 resides. The QuantumBirds consortium (Mouritsen, Hore, Winklhofer) produced this finding. Epistemic note: E-level for protein localization, L*-level for magnetoreception interpretation.",
    berm_relevance_fi: "KRIITTINEN BERM:n polku B:lle: Tunnistaa TOISEN CRY-järjestelmän ihmisen verkkokalvolla. CRY1 sinisissä tapeissa (sensorinen/fototransduktiivinen) on erillinen CRY2:sta gangliosoluissa (sirkadiaaninen). Tämä on järjestelmä johon iiriksen pigmentaatio vaikuttaa suorimmin — siniset silmät päästävät ~100x enemmän valoa nimenomaan sinisille tapeille joissa CRY1 sijaitsee. QuantumBirds-konsortio (Mouritsen, Hore, Winklhofer) tuotti tämän löydöksen. Episteeminen huomio: E-taso proteiinilokalisaatiolle, L*-taso magnetoreseptiotulkinnalle.",
    berm_relevance_ja: "BERMパスウェイBに極めて重要：ヒト網膜における第二のCRYシステムを同定。青色錐体のCRY1（感覚/光伝達）は神経節細胞のCRY2（概日リズム）とは異なる。これは虹彩色素沈着に最も直接的に影響されるシステムである――青い目はCRY1が存在する青色錐体に特異的に約100倍の光を透過する。QuantumBirdsコンソーシアム（Mouritsen, Hore, Winklhofer）がこの知見を生み出した。認識論的注記：タンパク質局在にはEレベル、磁気受容解釈にはL*レベル。",
    berm_relevance_fr: "CRITIQUE pour la voie BERM B : Identifie un SECOND système CRY dans la rétine humaine. CRY1 dans les cônes bleus (sensoriel/phototransductif) est distinct de CRY2 dans les cellules ganglionnaires (circadien). C'est le système le plus directement affecté par la pigmentation de l'iris — les yeux bleus transmettent ~100x plus de lumière spécifiquement aux cônes bleus où réside CRY1. Le consortium QuantumBirds (Mouritsen, Hore, Winklhofer) a produit cette découverte. Note épistémique : niveau E pour la localisation protéique, niveau L* pour l'interprétation magnétoréceptive.",
    berm_relevance_ko: "BERM 경로 B에 결정적: 인간 망막에서 두 번째 CRY 시스템을 확인한다. 청색 원추세포의 CRY1(감각/광전달)은 신경절세포의 CRY2(개일리듬)와 구별된다. 이것은 홍채 색소침착에 가장 직접적으로 영향받는 시스템이다 — 파란 눈은 CRY1이 존재하는 청색 원추세포에 특이적으로 약 100배 더 많은 빛을 투과시킨다. QuantumBirds 컨소시엄(Mouritsen, Hore, Winklhofer)이 이 발견을 산출하였다. 인식론적 주석: 단백질 국소화에는 E 수준, 자기수용 해석에는 L* 수준.",
  },
  {
    id: "majewska2025",
    ...
    berm_relevance_en: "Establishes E-level evidence for the omega fatty acid hypothesis: membrane lipid composition directly determines CRY protein orientation, which is a prerequisite for directional magnetic sensing. If membrane composition is altered by dietary fatty acid imbalance (high omega-6, low omega-3/7), CRY orientation may become randomized, reducing magnetoreceptive resolution. This connects nutritional status to pathway B (CRY1) function through a physical-structural mechanism.",
    berm_relevance_fi: "Vahvistaa E-tason näytön omega-rasvahappohypoteesille: kalvon lipidikoostumus määrittää suoraan CRY-proteiinin orientaation, joka on edellytys suuntakohtaiselle magneettiaistimukselle. Jos kalvon koostumus muuttuu ruokavalion rasvahappo-epätasapainosta (korkea omega-6, matala omega-3/7), CRY:n orientaatio voi muuttua satunnaiseksi, heikentäen magnetoreseptiivistä erottelukykyä.",
    berm_relevance_ja: "オメガ脂肪酸仮説のEレベル証拠を確立する：膜脂質組成がCRYタンパク質の配向を直接決定し、これは方向性磁気感知の前提条件である。食事性脂肪酸の不均衡（高オメガ6、低オメガ3/7）により膜組成が変化すると、CRYの配向がランダム化し、磁気受容の分解能が低下する可能性がある。物理構造的メカニズムを通じて栄養状態をパスウェイB (CRY1)機能に結びつける。",
    berm_relevance_fr: "Établit des preuves de niveau E pour l'hypothèse des acides gras oméga : la composition lipidique membranaire détermine directement l'orientation de la protéine CRY, qui est un prérequis pour la détection magnétique directionnelle. Si la composition membranaire est altérée par un déséquilibre alimentaire en acides gras (oméga-6 élevé, oméga-3/7 faible), l'orientation de CRY pourrait devenir aléatoire, réduisant la résolution magnétoréceptive. Cela connecte le statut nutritionnel à la fonction de la voie B (CRY1) par un mécanisme physico-structural.",
    berm_relevance_ko: "오메가 지방산 가설에 대한 E 수준 증거를 확립한다: 막 지질 조성이 CRY 단백질 배향을 직접 결정하며, 이는 방향성 자기 감지의 전제조건이다. 식이 지방산 불균형(높은 오메가-6, 낮은 오메가-3/7)으로 막 조성이 변경되면 CRY 배향이 무작위화되어 자기수용 해상도가 감소할 수 있다. 물리-구조적 메커니즘을 통해 영양 상태를 경로 B (CRY1) 기능에 연결한다.",
  },
  {
    id: "lamia2009",
  ...
  },
  {
    ...
    finding_fi: "Kolme itsenäistä näyttölinjaa: (1) Wacker ym. 2000: B2-puutteisilla äideillä oli 4,7-kertainen pre-eklampsian riski (OR 4,7, CI 1,8-12,2). (2) IVF-klinikat: B2-lisä parantaa alkionlaatumittareita. (3) Kiinassa on >90 % B2-puutos (CNHS 2015-2017) samanaikaisesti maailman alhaisimman TFR:n kanssa. B2-hedelmällisyysyhteys toimii FAD-riippuvaisen CRY-stabiilisuuden, FAD-riippuvaisen mitokondriaalisen toiminnan ja FAD-riippuvaisen folaattimetabolian kautta.",
    ...
    berm_relevance_en: "The China B2 case: China has >90% B2 inadequacy AND the world's lowest TFR. This is ecological correlation (not causal proof), but the mechanism is clear: B2 → FAD → CRY stability → pathway B function. This applies equally to conventional explanations. If B2 supplementation in China improved CRY-dependent circadian markers, it would be strong evidence for the nutritional modulation hypothesis. This is prediction NUT-2.",
    berm_relevance_fi: "Kiinan B2-tapaus: Kiinassa on >90 % B2-puutos JA maailman alhaisin TFR. Tämä on ekologinen korrelaatio (ei kausaalinen todiste), mutta mekanismi on selvä: B2 → FAD → CRY-stabiilisuus → polku B:n toiminta. Tämä koskee yhtä lailla konventionaalisia selityksiä. Jos B2-lisä Kiinassa parantaisi CRY-riippuvaisia sirkadiaanisia markkereita, se olisi vahvaa näyttöä ravitsemuksellisen modulaation hypoteesille. Tämä on ennuste NUT-2.",
    berm_relevance_ja: "中国B2事例：中国はB2摂取不足が>90%であり、かつ世界最低のTFRを持つ。これは生態学的相関（因果証明ではない）だが、メカニズムは明確：B2 → FAD → CRY安定性 → パスウェイB機能。これは従来の説明にも同様に当てはまる。中国でのB2補充がCRY依存性概日マーカーを改善すれば、栄養調節仮説の強力な証拠となる。これは予測NUT-2である。",
    berm_relevance_fr: "Le cas B2 de la Chine : la Chine a >90 % d'insuffisance en B2 ET le TFR le plus bas du monde. C'est une corrélation écologique (pas une preuve causale), mais le mécanisme est clair : B2 → FAD → stabilité CRY → fonction de la voie B. Cela s'applique également aux explications conventionnelles. Si la supplémentation en B2 en Chine améliorait les marqueurs circadiens dépendants de CRY, ce serait une preuve forte pour l'hypothèse de modulation nutritionnelle. C'est la prédiction NUT-2.",
    berm_relevance_ko: "중국 B2 사례: 중국은 B2 부족이 >90%이고 세계 최저 TFR을 가진다. 이는 생태학적 상관관계(인과적 증거가 아님)이지만 메커니즘은 명확하다: B2 → FAD → CRY 안정성 → 경로 B 기능. 이는 기존 설명에도 동일하게 적용된다. 중국에서 B2 보충이 CRY 의존성 개일리듬 마커를 개선한다면 영양 조절 가설의 강력한 증거가 될 것이다. 이것은 예측 NUT-2이다.",
  },
];
...
export const EYE_PROFILES: EyeProfile[] = [
  {
    ...
    label_ko: "파랑 — 최대 감수성",
    transmission: "Retinal light dose: to be measured (least iris melanin)",
  ...
  },
  {
    ...
    label_ko: "녹색 — 최적 안정성",
    transmission: "Retinal light dose: to be measured (lipochrome-filtered)",
  ...
  },
  {
    ...
    transmission: "Retinal light dose: to be measured (strong melanin)",
...
export const NUTRITIONAL_MODULATORS: NutritionalModulator[] = [
  ...
  },
  {
    ...
    deficiency_effect_en: "Retinal ganglion cells (where CRY resides) become vulnerable to oxidative damage from pathway A (VGCC → Ca²⁺ → ROS). Pathway A degrades pathway B's substrate.",
    deficiency_effect_fi: "Verkkokalvon gangliosolut (joissa CRY sijaitsee) tulevat haavoittuvaisiksi polku A:n oksidatiiviselle vauriolle (VGCC → Ca²⁺ → ROS). Polku A tuhoaa polku B:n substraattia.",
    deficiency_effect_ja: "網膜神経節細胞（CRYが存在する場所）がパスウェイA（VGCC → Ca²⁺ → ROS）による酸化損傷に対して脆弱になる。パスウェイAがパスウェイBの基質を分解する。",
    deficiency_effect_fr: "Les cellules ganglionnaires rétiniennes (où réside CRY) deviennent vulnérables aux dommages oxydatifs de la voie A (VGCC → Ca²⁺ → ROS). La voie A dégrade le substrat de la voie B.",
    deficiency_effect_ko: "망막 신경절세포(CRY가 존재하는 곳)가 경로 A(VGCC → Ca²⁺ → ROS)에 의한 산화적 손상에 취약해진다. 경로 A가 경로 B의 기질을 분해한다.",
...
export const PREDICTIONS: Prediction[] = [
  ...
  {
    id: "EYE-5",
    title_en: "Eye color modulates BERM pathway B effectiveness across populations",
    title_fi: "Silmien väri säätelee BERM:n polku B:n tehokkuutta populaatioiden välillä",
    title_ja: "眼の色は集団間でBERMパスウェイBの有効性を調節する",
    title_fr: "La couleur des yeux module l'efficacité de la voie BERM B entre les populations",
    title_ko: "눈 색깔은 집단 간 BERM 경로 B의 효과를 조절한다",
    test_en: "Ecological: in the 54-country BERM dataset, test whether populations with higher prevalence of blue/green eyes show stronger EMF-TFR association after controlling for GDP and education. If pathway B is modulated by eye color, northern European populations (high blue-eye prevalence) should show larger biological EMF sensitivity.",
    test_fi: "Ekologinen: BERM:n 54 maan datasetissä testaa osoittavatko populaatiot joissa on korkeampi sinisten/vihreiden silmien esiintyvyys voimakkaampaa EMF-TFR-assosiaatiota BKT:n ja koulutuksen kontrolloinnin jälkeen. Jos polku B:tä säätelee silmien väri, Pohjois-Euroopan populaatioiden (korkea sinisilmäisyyden esiintyvyys) tulisi osoittaa suurempaa biologista EMF-herkkyyttä.",
    test_ja: "生態学的：BERM 54カ国データセットにおいて、GDPと教育を統制した後、青色/緑色の目の有病率が高い集団がより強いEMF-TFR関連を示すかを検定する。パスウェイBが眼の色により調節されるならば、北欧集団（青い目の有病率が高い）はより大きな生物学的EMF感受性を示すはずである。",
    test_fr: "Écologique : dans le jeu de données BERM de 54 pays, tester si les populations avec une prévalence plus élevée d'yeux bleus/verts montrent une association EMF-TFR plus forte après contrôle du PIB et de l'éducation. Si la voie B est modulée par la couleur des yeux, les populations d'Europe du Nord (forte prévalence d'yeux bleus) devraient montrer une plus grande sensibilité biologique aux EMF.",
    test_ko: "생태학적: BERM 54개국 데이터셋에서 GDP와 교육을 통제한 후 파란/녹색 눈의 유병률이 높은 집단이 더 강한 EMF-TFR 연관을 보이는지 검정한다. 경로 B가 눈 색깔에 의해 조절된다면, 북유럽 집단(파란 눈 유병률이 높음)은 더 큰 생물학적 EMF 감수성을 보여야 한다.",
    discriminating: true,
  },
];
