...
export const EVIDENCE_CARDS: EvidenceCard[] = [
  ...
  },
  {
    ...
    berm_relevance_en: "The China B2 case: China has >90% B2 inadequacy AND the world's lowest TFR. This is ecological correlation (not causal proof), but the mechanism is clear: B2 → FAD → CRY stability → pathway B function. If B2 supplementation in China improved CRY-dependent circadian markers, it would be strong evidence for the nutritional modulation hypothesis. This is prediction NUT-2.",
    berm_relevance_fi: "Kiinan B2-tapaus: Kiinassa on >90 % B2-puutos JA maailman alhaisin TFR. Tämä on ekologinen korrelaatio (ei kausaalinen todiste), mutta mekanismi on selvä: B2 → FAD → CRY-stabiilisuus → polku B:n toiminta. Jos B2-lisä Kiinassa parantaisi CRY-riippuvaisia sirkadiaanisia markkereita, se olisi vahvaa näyttöä ravitsemuksellisen modulaation hypoteesille. Tämä on ennuste NUT-2.",
    berm_relevance_ja: "中国B2事例：中国はB2摂取不足が>90%であり、かつ世界最低のTFRを持つ。これは生態学的相関（因果証明ではない）だが、メカニズムは明確：B2 → FAD → CRY安定性 → パスウェイB機能。中国でのB2補充がCRY依存性概日マーカーを改善すれば、栄養調節仮説の強力な証拠となる。これは予測NUT-2である。",
    berm_relevance_fr: "Le cas B2 de la Chine : la Chine a >90 % d'insuffisance en B2 ET le TFR le plus bas du monde. C'est une corrélation écologique (pas une preuve causale), mais le mécanisme est clair : B2 → FAD → stabilité CRY → fonction de la voie B. Si la supplémentation en B2 en Chine améliorait les marqueurs circadiens dépendants de CRY, ce serait une preuve forte pour l'hypothèse de modulation nutritionnelle. C'est la prédiction NUT-2.",
    berm_relevance_ko: "중국 B2 사례: 중국은 B2 부족이 >90%이고 세계 최저 TFR을 가진다. 이는 생태학적 상관관계(인과적 증거가 아님)이지만 메커니즘은 명확하다: B2 → FAD → CRY 안정성 → 경로 B 기능. 중국에서 B2 보충이 CRY 의존성 개일리듬 마커를 개선한다면 영양 조절 가설의 강력한 증거가 될 것이다. 이것은 예측 NUT-2이다.",
  },
];
