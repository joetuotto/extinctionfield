...
const COPY = {
  en: {
    ...
    catsperLead: "Young’s human study locates CatSper-dependent hyperactivation as a functional fertilization gate: routine semen parameters can be normal while fertilization fails, and ICSI can bypass the defect. Calcium-dependent sperm production, transport and oocyte activation include other machinery; this result does not make CatSper necessary at every stage or in every species.",
    catsperP1: "CatSper integrates sperm voltage, pH, temperature and progesterone signaling. Its clinical importance is a functional gate: human deficiency can prevent hyperactivation despite normal routine semen parameters. BERM tests whether a recorded field changes the timing of calcium entry and clearance, ATP use and fertilization; a later low calcium value alone does not establish an earlier premature activation. Direct RF gating of human CatSper remains open until a specified field protocol links patch-clamp or matched calcium-response measurements to function.",
    catsperP2: "CATSPER2⁻/⁻ men show abolished progesterone-induced hyperactivation and fertilization failure ([[ref:catsper_human|JCI 2024]]), while NNC55-0396 blocks CatSper-dependent responses ([[ref:pmc6104424_nnc|Rennhack et al. 2018]]). This establishes necessity and an intervention handle, not equivalence between blocker and EMF exposure.",
    catsperEvolution: "CatSper-dependent sperm functions are demonstrated in humans and some other lineages, including sea urchins. Conservation does not establish identical machinery or guidance signals in all species; a salmon test first identifies the relevant calcium channels and physiological cues. Submarine-cable comparisons also require local dosimetry and species-specific fertilization endpoints.",
    ...
    sentinelGradient: "Across 7 species with quantifiable reproductive decline rates, cumulative reproductive decline correlates with estimated EMF burden at r = 0.84. Dogs ([[ref:lea2016|Lea et al. 2016]]): −1.0%/yr sperm decline over 26 years in UK stud dogs sharing domestic EMF exposure. Horses ([[ref:harris2023|Harris et al. 2023]]): −0.75%/yr stallion sperm decline over 35 years. The pattern extends to honeybees, birds, frogs, and aquatic species — each declining on timelines proportional to their electromagnetic environment.",
    sentinelTLink: "Cross-species trends motivate tests of conserved receptor, redox and repair machinery under matched local fields. Dogs share chemicals, diet, disease and breeding history as well as some human field environments. Their trends do not by themselves identify an EMF cause or calibrate a testosterone-to-TFR lag.",
    sentinelGradientStat: "r = 0.84",
    ...
    dualInterpretationLead: "Each finding is compared with specific mechanisms and the measurements that distinguish them. BERM retains nulls, protective effects and adverse effects as separate constraints; a missing background measurement is a data gap, not an automatic correction.",
    ...
    dualInterpretationRows: [
      {
        ...
        standard: "No detectable difference for this endpoint under the tested contrast; inspect precision and protocol.",
        berm: "Compare absolute sham levels, measured field/background and receiver history. Saturation predicts impaired baseline with reduced increment; adaptation predicts improved challenge tolerance; a true null remains possible.",
      },
      {
        evidence: "Study finds EMF effect at high SAR only",
        standard: "The tested high-dose effect needs thermal and nonthermal mechanisms separated.",
        berm: "Test temperature-matched dose and amplitude windows in the same system. A high-dose effect neither establishes nor excludes an effect at a lower dose.",
      },
      {
        evidence: "WHO systematic review rates certainty as 'moderate'",
        standard: "Certainty belongs to a specified endpoint, population and exposure range.",
        berm: "Keep corrected estimates and sensitivity analyses together. In the corrected male-fertility reconstruction, excluding the high-SAR study after risk-of-bias restriction changed the not-pregnant OR from 1.68 to 1.32, with the latter interval including 1; litter size was a separate endpoint.",
      },
      {
        evidence: "GDP correlates with TFR better than EMF proxy",
        standard: "A proxy association needs a causal model of development, exposure and reproductive opportunities.",
        berm: "State competing graphs before adjustment. GDP can proxy common causes, mediate a pathway or affect reproductive opportunities; its role is not fixed by its correlation with electrification.",
      },
      {
        evidence: "Study shows positive EMF effect (e.g. ROS increase)",
        standard: "Specify whether a ROS or calcium change is a signal, damage or a functional benefit.",
        berm: "Measure the proximal response, repair and final function separately. Report the observed sign and magnitude; a protective result is not reclassified as harm or automatically inflated for an unmeasured baseline.",
      },
      {
        evidence: "RPM cannot explain effects at telecom frequencies",
        standard: "Carrier-frequency constraints apply to the specified radical-pair mechanism.",
        berm: "Define how a receiver obtains a slow driver from the recorded signal. An RF envelope is not itself an ELF magnetic component. Compare window and alternative mechanisms against difficult CW-positive/GSM-null observations.",
      },
      {
    ...
    theraBionicTitle: "Protocol-specific clinical anchor: TheraBionic",
    theraBionicLead: "TheraBionic constrains one carrier × envelope × channel × organ route. It does not validate BERM's environmental or reproductive transfer.",
    theraBionicBody: "TheraBionic P1 received FDA Humanitarian Device Exemption approval (H220001) on 26 September 2023 for advanced hepatocellular carcinoma and uses amplitude-modulated radiofrequency fields at 27.12 MHz.",
    theraBionicMechanism: "Jimenez et al. (2019) reported Cav3.2-dependent calcium influx and differentiation in HCC models under the specified protocol. This is direct component evidence for that system, not proof of a universal BERM tissue kernel.",
    theraBionicSAR: "The reported low-SAR protocol shows that a specified non-thermal exposure can be biologically active in this system. Relative SAR alone does not transfer its dose, sign or endpoint to another waveform, organ or population.",
    theraBionicCCB: "The FDA label advises against use with L- or T-type calcium-channel blockers unless therapy is modified. The label is a protocol constraint consistent with the channel route, not a randomized blocker experiment.",
    theraBionicImplication: "The device is independent component evidence for a protocol-specific Cav3.2 route. Environmental exposure, reproductive tissue and population outcomes require separate transfer and calibration tests.",
    theraBionicSurvival: "14/41 with stable disease beyond 6 months",
    theraBionicDevice: "27.12 MHz AM-RF, tumor-specific frequencies",
    theraBionicChannel: "Cav3.2 (CACNA1H) T-type VGCC",
    theraBionicLevel: "Direct component — FDA HDE 2023 + peer-reviewed HCC mechanism",
    ...
    tDeclineBmiNote: "BMI can be a mediator, confounder or time-varying consequence, depending on the declared causal graph. Weight-stable cohorts help show that weight change alone is insufficient, but they do not identify an EMF cause or a fixed direct/indirect fraction. Compare competing adjustment sets and report which estimand each addresses.",
    ...
    metabLead: "Metabolic branches converge through calcium handling, hepatic glucose production, insulin demand and tissue-clock timing. Measure each functional flow and the shared feedback before estimating a field contribution to energy balance; the branches are not six independent causal effect sizes.",
    metabStudies: [
      ...
      { referenceId: "bhatt2012_glp1", authors: "Selway et al.", year: 2012, journal: "PLoS ONE", finding: "GLP-1 activates ERK via L-type VGCC Ca²⁺ microdomain in β-cells", mechanism: "3: Insulin", level: "E" },
    ...
    svgVsZero: "vs. assumed reference",
    ...
    solarNorthernTraits: [
      "Iris pigmentation → measured retinal light dose → χ_optical (the dose is the input, not a transmission multiplier)",
    ...
    plantCryTitle: "Plants Respond to EMF Through the Same Molecule",
    plantCryIntro: "Same molecule (CRY), same cofactor (FAD), same mechanism (RPM) regulates reproduction in plants (flowering) and animals (melatonin → HPG axis). Plant cryptochrome research provides the cleanest test of the radical-pair mechanism because plants synthesize their own FAD — removing dietary B2 as a confound.",
    plantCryResearchLabel: "Plant CRY research evidence",
    plantCryStudies: [
      { authors: "Ahmad et al.", year: "2020", finding: "7 MHz RF reduces CRY1 response in Arabidopsis — RPM fingerprint in plant cryptochrome", mechanism: "RPM direct test" },
      { authors: "Xu et al.", year: "2014", finding: "500 µT static field enhances CRY phosphorylation in Arabidopsis", mechanism: "CRY activation" },
      { authors: "Xu et al.", year: "2015", finding: "Near-null magnetic field suppresses flowering via CRY pathway", mechanism: "CRY-dependent flowering" },
      { authors: "Agliassa et al.", year: "2018", finding: "Geomagnetic field modulates CRY-dependent signaling in Arabidopsis", mechanism: "CRY signaling" },
      { authors: "PMC10005510", year: "2023", finding: "Wi-Fi frequencies (2.4 GHz) accelerate lettuce flowering time", mechanism: "RF → flowering" },
      { authors: "Ecological Indicators", year: "2023", finding: "868 MHz RF exposure affects growth and reproduction in 10 wild plant species", mechanism: "RF → reproduction" },
      { authors: "Haggerty", year: "2010", finding: "RF background from cell towers associated with decline in nearby aspen seedlings", mechanism: "RF → growth" },
    ],
    plantCryB2Title: "The B2/FAD Asymmetry: Why Plants Are the Cleaner Test",
    plantCryB2Plants: "PLANTS synthesize their own B2 → FAD supply is endogenous → CRY function depends ONLY on RF disruption → Ahmad 2020’s \"relatively minor\" effect is a PURE RPM test",
    plantCryB2Animals: "ANIMALS require dietary B2 → CRY function depends on BOTH RF AND B2 status → DOUBLE vulnerability (RF disrupts radical pairs + B2 deficiency starves the cofactor)",
    mastingTitle: "Masting: Synchronized Reproduction Across 2,000 km",
    mastingIntro: "Masting — the synchronized mass seeding of forest trees — is one of ecology’s most striking coordination phenomena. Trees across continental distances produce bumper seed crops in the same year, then near-zero in others. The synchronization signal must be detectable identically across 2,000+ km.",
    mastingBermExplanation: "CRY2 reads photoperiod, temperature AND geomagnetic field simultaneously. Of these three signals, only the geomagnetic field is effectively identical at 2,000 km scale — photoperiod varies with latitude, temperature varies locally. BERM predicts that the geomagnetic component of CRY2-mediated synchronization is being disrupted by growing RF background, weakening masting coherence.",
    mastingResearchLabel: "Masting research evidence",
    mastingStudies: [
      { authors: "Nature Plants", year: "2024", finding: "Summer solstice acts as a ‘celestial starting gun’ — photoperiod signal triggers synchronized masting across 2,000 km in European beech", mechanism: "Synchronization signal" },
      { authors: "Bogdziewicz et al.", year: "2021", finding: "Masting synchrony is weakening across European forests — attributed to climate change, but BERM alternative: RF background growth disrupts CRY2 geomagnetic sensing", mechanism: "Synchrony loss" },
    ...
    horm_exp_note: "Ionizing-radiation adaptation anchors repair biology in its own dose class. RF preconditioning with an autophagy intervention supplies a closer field-specific comparison; the two exposures do not share an assumed dose scale.",
    ...
    horm_gen_note: "The observed genotype–environment association motivates a context-dependent functional model. Settlement and BMI do not measure a local electromagnetic dose or a genotype-specific field coefficient.",
    ...
    horm_neuro_note: "Separate hormone amount from pulse timing and tissue receptivity. A prospective urban comparison should measure local fields, sleep, social stressors and the received endocrine response before assigning an environmental mechanism.",
    horm_epistemic: "BERM represents beneficial adaptation, impaired reception and injury as distinguishable state trajectories. The sign is tested with baseline function, repair and a defined challenge; “hormesis” does not select the explanation after the outcome is known.",
    ...
    catsperLead: "Youngin ihmistutkimus paikantaa CatSper-riippuvaisen hyperaktivaation hedelmöityksen toiminnalliseksi portiksi: rutiiniparametrit voivat olla normaaleja hedelmöityksen epäonnistuessa, ja ICSI voi ohittaa puutteen. Kalsiumriippuvainen siittiötuotanto, kulkeutuminen ja munasolun aktivaatio sisältävät muutakin koneistoa; tulos ei tee CatSperia välttämättömäksi jokaisessa vaiheessa tai kaikissa lajeissa.",
    catsperP1: "CatSper yhdistää siittiön jännitteen, pH:n, lämpötilan ja progesteronisignaloinnin. Sen kliininen merkitys on toiminnallinen portti: ihmisen puutos voi estää hyperaktivaation normaaleista rutiiniparametreista huolimatta. BERM testaa, muuttaako tallennettu kenttä kalsiumin sisäänvirtauksen ja poistuman ajoitusta, ATP:n käyttöä ja hedelmöitystä; myöhempi matala kalsiumarvo ei yksin osoita aiempaa ennenaikaista aktivaatiota. Ihmisen CatSperin suora RF-portitus on avoin, kunnes määritelty kenttäprotokolla yhdistää patch-clamp- tai sovitetut kalsiumvastemittaukset toimintoon.",
    catsperP2: "CATSPER2⁻/⁻-miehiltä puuttuu progesteronin indusoima hyperaktivaatio ja hedelmöitys epäonnistuu ([[ref:catsper_human|JCI 2024]]), kun taas NNC55-0396 salpaa CatSper-riippuvaisia vasteita ([[ref:pmc6104424_nnc|Rennhack ym. 2018]]). Tämä osoittaa välttämättömyyden ja interventiokohdan, ei salpaajan ja EMF-altistuksen ekvivalenssia.",
    catsperEvolution: "CatSper-riippuvaisia siittiötoimintoja on osoitettu ihmisessä ja joissakin muissa kehityslinjoissa, kuten merisiileillä. Säilyminen ei osoita samaa koneistoa tai ohjaussignaaleja kaikissa lajeissa; lohikoe tunnistaa ensin olennaiset kalsiumkanavat ja fysiologiset signaalit. Merikaapelivertailut tarvitsevat myös paikallisen dosimetrian ja lajikohtaiset hedelmöityspäätepisteet.",
    ...
    sentinelGradient: "Seitsemässä lajissa, joilla on kvantifioitava lisääntymislaskuaste, kumulatiivinen lisääntymislasku korreloi arvioidun EMF-kuorman kanssa tasolla r = 0,84. Koirat ([[ref:lea2016|Lea ym. 2016]]): −1,0 %/v siittiölasku 26 vuoden aikana brittiläisissä siitoskoirissa, jotka jakavat kodin EMF-altistuksen. Hevoset ([[ref:harris2023|Harris ym. 2023]]): −0,75 %/v oriiden siittiölasku 35 vuoden aikana. Kuvio ulottuu mehiläisiin, lintuihin, sammakoihin ja vesilajeihin — kukin laskee aikajanalla, joka on suhteessa niiden sähkömagneettiseen ympäristöön.",
    sentinelTLink: "Lajienväliset trendit motivoivat säilyneen vastaanotto-, redox- ja korjauskoneiston testaamista vertailukelpoisissa paikallisissa kentissä. Koirat jakavat ihmisten kanssa kemikaaleja, ravintoa, sairauksia ja jalostushistoriaa sekä osan kenttäympäristöistä. Trendit eivät yksin tunnista EMF-syytä eivätkä kalibroi testosteronin ja TFR:n välistä viivettä.",
    sentinelGradientStat: "r = 0,84",
    ...
    dualInterpretationLead: "Jokaista havaintoa verrataan nimettyihin mekanismeihin ja niitä erotteleviin mittauksiin. BERM säilyttää nollatulokset, suojaavat vaikutukset ja haitat erillisinä rajoitteina; puuttuva taustamittaus on tietovaje, ei automaattinen korjaus.",
    ...
    dualInterpretationRows: [
      {
        ...
        standard: "Ei havaittavaa eroa tämän päätepisteen testatussa kontrastissa; tarkasta tarkkuus ja koeasetelma.",
        berm: "Vertaa sham-ryhmän absoluuttista tasoa, mitattua kenttää/taustaa ja vastaanotinhistoriaa. Kyllästyminen ennustaa heikentynyttä lähtötasoa ja pienempää lisävastetta; adaptaatio parempaa haastetoleranssia; myös todellinen nollatulos on mahdollinen.",
      },
      {
        evidence: "Tutkimus löytää EMF-vaikutuksen vain korkealla SAR:lla",
        standard: "Testatun suuren annoksen vaikutuksessa on erotettava lämpö- ja muut mekanismit.",
        berm: "Testaa lämpötilaltaan vakioitu annos- ja amplitudi-ikkuna samassa järjestelmässä. Suuren annoksen vaikutus ei osoita eikä sulje pois vaikutusta pienemmällä annoksella.",
      },
      {
        evidence: "WHO:n systemaattinen katsaus arvioi varmuuden 'kohtalaiseksi'",
        standard: "Varmuus koskee määriteltyä päätepistettä, populaatiota ja altistusaluetta.",
        berm: "Säilytä korjatut arviot ja herkkyysanalyysit yhdessä. Korjatun lisääntymisaineiston rekonstruktiossa suuren SAR:n kokeen poisto harhariskirajauksen jälkeen muutti tiinehtymättä jäämisen OR:n 1,68:sta 1,32:een, jonka luottamusväli sisälsi arvon 1; poikuekoko oli erillinen päätepiste.",
      },
      {
        evidence: "BKT korreloi TFR:n kanssa paremmin kuin EMF-proxy",
        standard: "Sijaismuuttujan yhteys tarvitsee kausaalimallin kehityksestä, altistuksesta ja lisääntymismahdollisuuksista.",
        berm: "Määrittele kilpailevat kaaviot ennen vakiointia. BKT voi kuvata yhteisiä syitä, välittää reittiä tai vaikuttaa lisääntymismahdollisuuksiin; sen rooli ei määräydy sähköistymiskorrelaatiosta.",
      },
      {
        evidence: "Tutkimus näyttää positiivisen EMF-vaikutuksen (esim. ROS-nousu)",
        standard: "Määrittele, onko ROS- tai kalsiummuutos signaali, vaurio vai toiminnallinen hyöty.",
        berm: "Mittaa lähivaste, korjaus ja lopullinen toiminto erikseen. Raportoi havaittu suunta ja suuruus; suojaavaa tulosta ei nimetä haitaksi eikä suurenneta automaattisesti mittaamattoman lähtötason perusteella.",
      },
      {
        evidence: "RPM ei voi selittää vaikutuksia telecom-taajuuksilla",
        standard: "Kantajataajuuden rajoitteet koskevat määriteltyä radikaaliparimekanismia.",
        berm: "Määrittele, miten vastaanotin saa hitaan ajurin tallennetusta signaalista. RF-verhokäyrä ei itsessään ole ELF-magneettikomponentti. Vertaa ikkuna- ja vaihtoehtoisia mekanismeja myös vaikeisiin CW-positiivisiin/GSM-nollahavaintoihin.",
    ...
    theraBionicTitle: "Protokollikohtainen kliininen ankkuri: TheraBionic",
    theraBionicLead: "TheraBionic rajaa yhtä kantoaalto × verhokäyrä × kanava × elin -reittiä. Se ei validoi BERM:n ympäristö- tai lisääntymissiirtoa.",
    theraBionicBody: "TheraBionic P1 sai FDA:n Humanitarian Device Exemption -hyväksynnän (H220001) 26.9.2023 edenneeseen maksasolukarsinoomaan ja käyttää 27,12 MHz:n amplitudimoduloitua radiotaajuutta.",
    theraBionicMechanism: "Jimenez ym. (2019) raportoivat määritellyssä HCC-protokollassa Cav3.2-riippuvaisen kalsiumvirtauksen ja differentiaation. Tämä on suoraa komponenttinäyttöä kyseisestä järjestelmästä, ei universaalin BERM-kudosytimen todistus.",
    theraBionicSAR: "Raportoitu matalan SAR:n protokolla osoittaa, että määritelty ei-terminen altistus voi olla tässä järjestelmässä biologisesti aktiivinen. Suhteellinen SAR ei yksin siirrä annosta, etumerkkiä tai päätepistettä toiseen aaltomuotoon, elimeen tai populaatioon.",
    theraBionicCCB: "FDA-merkintä neuvoo välttämään käyttöä L- tai T-tyypin kalsiumkanavasalpaajien kanssa, ellei hoitoa muuteta. Merkintä on kanavareitin kanssa yhteensopiva protokollarajoite, ei satunnaistettu salpaajakoe.",
    theraBionicImplication: "Laite on itsenäistä komponenttinäyttöä protokollikohtaisesta Cav3.2-reitistä. Ympäristöaltistus, lisääntymiskudos ja populaatiotulokset vaativat erilliset siirto- ja kalibrointitestit.",
    theraBionicSurvival: "14/41: tauti vakaana yli 6 kuukautta",
    theraBionicDevice: "27,12 MHz AM-RF, kasvainspesifiset taajuudet",
    theraBionicChannel: "Cav3.2 (CACNA1H) T-tyypin VGCC",
    theraBionicLevel: "Suora komponentti — FDA HDE 2023 + vertaisarvioitu HCC-mekanismi",
    ...
    tDeclineBmiNote: "BMI voi olla välittäjä, sekoittaja tai ajassa muuttuva seuraus ilmoitetusta kausaalikaaviosta riippuen. Painoltaan vakaat kohortit auttavat osoittamaan, ettei painonmuutos yksin riitä selitykseksi, mutta ne eivät tunnista EMF-syytä tai kiinteää suoraa/välillistä osuutta. Vertaa kilpailevia vakiointeja ja ilmoita kunkin arvioima suure.",
    ...
    metabLead: "Aineenvaihduntahaarat yhdistyvät kalsiuminkäsittelyn, maksan glukoosintuotannon, insuliinikysynnän ja kudoskellojen ajoituksen kautta. Mittaa kukin toiminnallinen virta ja yhteinen palaute ennen kentän osuuden estimointia energiatasapainossa; haarat eivät ole kuusi riippumatonta kausaalista vaikutuskokoa.",
    metabStudies: [
      ...
      { referenceId: "bhatt2012_glp1", authors: "Selway ym.", year: 2012, journal: "PLoS ONE", finding: "GLP-1 aktivoi ERK:n L-tyypin VGCC:n Ca²⁺-mikrodomeenin kautta β-soluissa", mechanism: "3: Insuliini", level: "E" },
    ...
    svgVsZero: "oletettuun vertailutilaan",
    ...
    solarNorthernTraits: [
      "Iiriksen pigmentaatio → mitattu verkkokalvon valoannos → χ_optinen (annos on syöte, ei läpäisykerroin)",
    ...
    plantCryTitle: "Kasvit reagoivat EMF:ään saman molekyylin kautta",
    plantCryIntro: "Sama molekyyli (CRY), sama kofaktori (FAD), sama mekanismi (RPM) säätelee lisääntymistä kasveissa (kukinta) ja eläimissä (melatoniini → HPG-akseli). Kasvi-kryptokromitutkimus tarjoaa puhtaimman testin radikaalipari-mekanismille, koska kasvit syntetisoivat oman FAD:nsa — poistaen ravinnon B2:n sekoittavana tekijänä.",
    plantCryResearchLabel: "Kasvi-CRY-tutkimusnäyttö",
    plantCryStudies: [
    ...
    plantCryB2Title: "B2/FAD-asymmetria: Miksi kasvit ovat puhtaampi testi",
    plantCryB2Plants: "KASVIT syntetisoivat oman B2:nsa → FAD-tarjonta on endogeeninen → CRY-toiminta riippuu VAIN RF-häiriöstä → Ahmad 2020:n \"suhteellisen vähäinen\" vaikutus on PUHDAS RPM-testi",
    plantCryB2Animals: "ELÄIMET tarvitsevat ravinnon B2:ta → CRY-toiminta riippuu SEKÄ RF:stä ETTÄ B2-statuksesta → KAKSINKERTAINEN haavoittuvuus (RF häiritsee radikaalipareja + B2-puutos nälkiinnyttää kofaktorin)",
    mastingTitle: "Masting: Synkronoitu reproduktio 2 000 km:n skaalalla",
    mastingIntro: "Masting — metsäpuiden synkronoitu massiivinen siementuotanto — on yksi ekologian hämmästyttävimmistä koordinaatioilmiöistä. Puut mantereiden laajuisilla etäisyyksillä tuottavat ennätyssiemensatoja samana vuonna, sitten lähes nollan toisina. Synkronointisignaalin on oltava havaittavissa identtisenä yli 2 000 km:n etäisyydellä.",
    mastingBermExplanation: "CRY2 lukee valopituuden, lämpötilan JA geomagneettisen kentän samanaikaisesti. Näistä kolmesta signaalista vain geomagneettinen kenttä on käytännössä identtinen 2 000 km:n skaalalla — valopituus vaihtelee leveysasteen mukaan, lämpötila paikallisesti. BERM ennustaa, että CRY2-välitteisen synkronoinnin geomagneettista komponenttia häiritsee kasvava RF-tausta, mikä heikentää masting-koherenssia.",
    mastingResearchLabel: "Masting-tutkimusnäyttö",
    mastingStudies: [
      { authors: "Nature Plants", year: "2024", finding: "Kesäpäivänseisaus toimii 'taivaallisena lähtölaukauksena' — valopituussignaali laukaisee synkronoidun mastingin 2 000 km:n matkalla eurooppalaisessa pyökissä", mechanism: "Synkronointisignaali" },
      { authors: "Bogdziewicz ym.", year: "2021", finding: "Masting-synkronia heikkenee Euroopan metsissä — selitetty ilmastonmuutoksella, mutta BERM-vaihtoehto: RF-taustan kasvu häiritsee CRY2:n geomagneettista aistimista", mechanism: "Synkronian heikkeneminen" },
    ...
    bioem_bertagna_text: "Bertagna ym. 2025 altistivat hiiren CA1-hermosoluja 50 Hz:n, 1 mT:n kentälle 60 minuutiksi ja raportoivat sisään- ja transientin ulosvirran vähenemistä. RyR-vapautuksen tai SERCA-takaisinoton häirintä poisti vaikutukset. Tämä paikantaa kytketyn kalsiumvarastomekanismin ilmoitetussa ELF-asetelmassa. Se motivoi alkukalsiumin, ER-varastotilan ja myöhemmän kalvovirran erillisiä mittauksia kahden riippumattoman reittipainon sijaan.",
    bioem_bertagna_level: "E",
    ...
    horm_exp_note: "Ionisoivan säteilyn adaptaatio ankkuroi korjausbiologiaa omassa annosluokassaan. RF-esikäsittely autofagiainterventiolla antaa läheisemmän kenttäkohtaisen vertailun; altisteille ei oleteta yhteistä annosasteikkoa.",
    ...
    horm_gen_note: "Havaittu genotyyppi–ympäristö-yhteys motivoi kontekstiriippuvaista toimintomallia. Asuinmuoto ja BMI eivät mittaa paikallista sähkömagneettista annosta tai genotyyppikohtaista kenttäkerrointa.",
    ...
    horm_neuro_note: "Erota hormonimäärä pulssien ajoituksesta ja kudoksen vastaanottavuudesta. Prospektiivisen kaupunkivertailun tulee mitata paikalliset kentät, uni, sosiaaliset kuormitukset ja vastaanotettu hormonivaste ennen ympäristömekanismin kohdistamista.",
    horm_epistemic: "BERM kuvaa hyödyllisen adaptaation, heikentyneen vastaanoton ja vaurion erotettavina tilakulkuina. Etumerkki testataan lähtötoiminnolla, korjauksella ja määritellyllä haasteella; ”hormeesi” ei valitse selitystä tuloksen tuntemisen jälkeen.",
  ...
  ja: {
    ...
    catsperP1: "CatSperは電位、pH、温度、プロゲステロンで調節される精子特異的チャネルである。BERMのEMF → CatSper段階は、指定フィールドプロトコルによる直接データが得られるまで未解決である。",
    catsperP2: "CATSPER2欠損とNNC55-0396は必要性と介入点を確立するが、遮断薬とEMF曝露の同等性は確立しない（[[ref:catsper_human|JCI 2024]]; [[ref:pmc6104424_nnc|Rennhack et al. 2018]]）。",
    ...
    sentinelGradient: "定量可能な生殖低下率を持つ7種にわたり、低下率はEMF曝露スコアとr = 0.84で相関する。犬（[[ref:lea2016|Lea et al. 2016]]）：英国種犬で26年間にわたり精子−1.0%/年の低下、家庭のEMF曝露を共有。馬（[[ref:harris2023|Harris et al. 2023]]）：種馬の精子35年間にわたり−0.75%/年の低下。パターンはミツバチ、鳥類、カエル、水生種に及ぶ — それぞれが電磁環境に比例したタイムラインで低下。",
    sentinelTLink: "種間の傾向は比較実験を動機づけますが、共有する化学物質、食事、疾病、繁殖歴も考慮が必要です。傾向だけではEMF原因やT–TFR遅延を特定できません。",
    sentinelGradientStat: "r = 0.84",
    ...
    theraBionicTitle: "プロトコル固有の臨床アンカー：TheraBionic",
    theraBionicLead: "一つのキャリア×包絡線×チャネル×臓器経路を制約するが、環境・生殖への移行は検証しない。",
    theraBionicBody: "TheraBionic P1は2023年9月26日にFDA HDE H220001承認を受けた27.12 MHz振幅変調装置である。",
    theraBionicMechanism: "2019年のHCC研究は指定プロトコルでCav3.2依存性Ca²⁺流入を報告した。これは構成要素証拠であり、普遍的なBERM組織カーネルの証明ではない。",
    theraBionicSAR: "低SARプロトコルの生物活性は、このシステムに限定される。SAR比だけで別の波形、臓器、集団へ線量や符号を移せない。",
    theraBionicCCB: "遮断薬に関するFDA表示は経路と整合するプロトコル制約であり、無作為化遮断薬実験ではない。",
    theraBionicImplication: "環境曝露、生殖組織、集団結果には別個の移行・校正試験が必要である。",
    theraBionicSurvival: "14/41で6か月超の病勢安定",
    theraBionicDevice: "27.12 MHz AM-RF、腫瘍特異的周波数",
    theraBionicChannel: "Cav3.2 (CACNA1H) T型VGCC",
    theraBionicLevel: "直接構成要素 — FDA HDE 2023 + 査読済みHCC機構",
    ...
    metabStudies: [
      ...
      { referenceId: "bhatt2012_glp1", authors: "Selway et al.", year: 2012, journal: "PLoS ONE", finding: "GLP-1がL型 VGCC Ca²⁺マイクロドメインを介してβ細胞でERKを活性化", mechanism: "3: インスリン", level: "E" },
    ...
    cry2PathwayNote: "CRY2の下流効果は概日時計を超えて拡がる。Iversen et al. (2025) は、CRY2がTRPファミリーのカチオンチャネルであるTRPC1と物理的に相互作用し、PEMF曝露後にこの複合体が共に核へ移行することを示した。このカルシウム流入経路はCRY2依存性（CRY2サイレンシングにより遮断）、光依存性（暗所で消失）、FAD依存性（RFKサイレンシングにより減衰）であり、すべてRPMメカニズムの特徴である。重要なことに、TRPC1は電位依存性カルシウムチャネルではなく、L型VGCC遮断薬では遮断されない。これは経路AとBが薬理学的に分離可能であり続けることを意味するが、経路Bの生物学的影響範囲は以前想定されていたよりも大きい。",
    ...
    solarNorthernTraits: [
      "虹彩色素沈着 → 測定された網膜光量 → χ_光学（入力は透過倍率ではなく光量そのもの）",
    ...
    plantCryTitle: "植物は同じ分子を通じてEMFに応答する",
    plantCryIntro: "同じ分子（CRY）、同じ補因子（FAD）、同じメカニズム（RPM）が植物（開花）と動物（メラトニン → HPG軸）の生殖を制御する。植物クリプトクロム研究はラジカルペア・メカニズムの最も純粋なテストを提供する — 植物は自らFADを合成するため、食事性B2が交絡因子にならない。",
    plantCryResearchLabel: "植物CRY研究エビデンス",
    plantCryStudies: [
      { authors: "Ahmad et al.", year: "2020", finding: "7 MHz RFがシロイヌナズナのCRY1応答を低下 — 植物クリプトクロムにおけるRPMフィンガープリント", mechanism: "RPM直接テスト" },
      { authors: "Xu et al.", year: "2014", finding: "500 µT静磁場がシロイヌナズナのCRYリン酸化を促進", mechanism: "CRY活性化" },
      { authors: "Xu et al.", year: "2015", finding: "ほぼゼロ磁場がCRY経路を介して開花を抑制", mechanism: "CRY依存性開花" },
      { authors: "Agliassa et al.", year: "2018", finding: "地磁気がシロイヌナズナのCRY依存性シグナル伝達を調節", mechanism: "CRYシグナル伝達" },
      { authors: "PMC10005510", year: "2023", finding: "Wi-Fi周波数（2.4 GHz）がレタスの開花を加速", mechanism: "RF → 開花" },
      { authors: "Ecological Indicators", year: "2023", finding: "868 MHz RF曝露が10種の野生植物の成長と繁殖に影響", mechanism: "RF → 繁殖" },
      { authors: "Haggerty", year: "2010", finding: "基地局からのRFバックグラウンドが近隣のアスペン実生の衰退と関連", mechanism: "RF → 成長" },
    ],
    plantCryB2Title: "B2/FAD非対称性：なぜ植物がよりクリーンなテストか",
    plantCryB2Plants: "植物は自らB2を合成 → FAD供給は内因性 → CRY機能はRF撹乱のみに依存 → Ahmad 2020の「比較的軽微な」効果は純粋なRPMテスト",
    plantCryB2Animals: "動物は食事性B2を必要 → CRY機能はRFとB2状態の両方に依存 → 二重脆弱性（RFがラジカルペアを撹乱 + B2欠乏が補因子を枯渇）",
    mastingTitle: "マスティング：2,000kmにわたる同期した繁殖",
    mastingIntro: "マスティング — 森林樹木の同期的大量結実 — は生態学で最も印象的な協調現象の一つである。大陸規模の距離にわたる樹木が同じ年に豊作を生み、他の年にはほぼゼロとなる。同期信号は2,000 km以上で同一に検出可能でなければならない。",
    mastingBermExplanation: "CRY2は光周期、温度、そして地磁気を同時に読み取る。この3つの信号のうち、地磁気のみが2,000 kmスケールで事実上同一である — 光周期は緯度で変化し、温度は局所的に変化する。BERMは、CRY2を介した同期の地磁気成分が増大するRFバックグラウンドにより撹乱され、マスティングのコヒーレンスが弱まると予測する。",
    mastingResearchLabel: "マスティング研究エビデンス",
    mastingStudies: [
      { authors: "Nature Plants", year: "2024", finding: "夏至が「天体のスターティングガン」として機能 — 光周期信号がヨーロッパブナで2,000 kmにわたる同期マスティングを誘発", mechanism: "同期信号" },
      { authors: "Bogdziewicz et al.", year: "2021", finding: "ヨーロッパの森林でマスティング同期が弱まっている — 気候変動に帰因されるが、BERM代替案：RFバックグラウンドの増大がCRY2の地磁気感知を撹乱", mechanism: "同期の喪失" },
  ...
  fr: {
    ...
    catsperP1: "CatSper est régulé par le voltage, le pH, la température et la progestérone. L'étape EMF → CatSper de BERM reste ouverte jusqu'à l'obtention de données directes sous un protocole de champ spécifié.",
    catsperP2: "La perte de CATSPER2 et NNC55-0396 établissent la nécessité et une cible d'intervention, mais pas l'équivalence entre bloqueur et exposition EMF ([[ref:catsper_human|JCI 2024]]; [[ref:pmc6104424_nnc|Rennhack et al. 2018]]).",
    ...
    sentinelGradient: "Sur 7 espèces avec des taux de déclin reproductif quantifiables, le taux de déclin est corrélé au score d'exposition EMF à r = 0,84. Chiens ([[ref:lea2016|Lea et al. 2016]]) : −1,0 %/an de déclin spermatique sur 26 ans chez les chiens reproducteurs britanniques partageant l'exposition EMF domestique. Chevaux ([[ref:harris2023|Harris et al. 2023]]) : −0,75 %/an de déclin spermatique des étalons sur 35 ans. Le schéma s'étend aux abeilles, oiseaux, grenouilles et espèces aquatiques — chacun déclinant sur des chronologies proportionnelles à leur environnement électromagnétique.",
    sentinelTLink: "Les tendances interespèces motivent des expériences comparables. Les substances chimiques, l’alimentation, les maladies et la sélection sont aussi partagées; ces tendances seules ne déterminent ni une cause EMF ni un délai T–TFR.",
    sentinelGradientStat: "r = 0,84",
    ...
    theraBionicTitle: "Ancrage clinique propre au protocole : TheraBionic",
    theraBionicLead: "Contraint une voie porteuse × enveloppe × canal × organe, sans valider le transfert environnemental ou reproductif.",
    theraBionicBody: "TheraBionic P1 a reçu l'autorisation FDA HDE H220001 le 26 septembre 2023 et utilise une radiofréquence modulée à 27,12 MHz.",
    theraBionicMechanism: "L'étude CHC de 2019 rapporte une entrée Ca²⁺ dépendante de Cav3.2 sous le protocole spécifié. C'est une preuve composante, pas la validation d'un noyau tissulaire BERM universel.",
    theraBionicSAR: "L'activité du protocole à faible DAS est limitée à ce système; le rapport de DAS ne transfère pas à lui seul dose, signe ou résultat à un autre organe.",
    theraBionicCCB: "La mise en garde FDA sur les inhibiteurs est une contrainte de protocole compatible avec la voie, non une expérience randomisée avec bloqueur.",
    theraBionicImplication: "L'exposition environnementale, les tissus reproducteurs et les résultats populationnels exigent des tests de transfert distincts.",
    theraBionicSurvival: "14/41 stables au-delà de 6 mois",
    theraBionicDevice: "27,12 MHz AM-RF, fréquences spécifiques à la tumeur",
    theraBionicChannel: "Cav3.2 (CACNA1H) VGCC de type T",
    theraBionicLevel: "Composante directe — FDA HDE 2023 + mécanisme CHC évalué par les pairs",
    ...
    metabStudies: [
      ...
      { referenceId: "bhatt2012_glp1", authors: "Selway et al.", year: 2012, journal: "PLoS ONE", finding: "GLP-1 active ERK via le microdomaine Ca²⁺ du VGCC de type L dans les cellules β", mechanism: "3 : Insuline", level: "E" },
    ...
    svgVsZero: "vs. référence supposée",
    ...
    solarNorthernTraits: [
      "Pigmentation de l'iris → dose lumineuse rétinienne mesurée → χ_optique (la dose est l'entrée, non un multiplicateur de transmission)",
    ...
    plantCryTitle: "Les plantes répondent aux EMF par la même molécule",
    plantCryIntro: "Même molécule (CRY), même cofacteur (FAD), même mécanisme (RPM) régule la reproduction chez les plantes (floraison) et les animaux (mélatonine → axe HPG). La recherche sur le cryptochrome végétal fournit le test le plus pur du mécanisme des paires radicalaires, car les plantes synthétisent leur propre FAD — éliminant la B2 alimentaire comme facteur confondant.",
    plantCryResearchLabel: "Preuves de recherche sur le CRY végétal",
    plantCryStudies: [
      { authors: "Ahmad et al.", year: "2020", finding: "RF 7 MHz réduit la réponse CRY1 chez Arabidopsis — empreinte RPM dans le cryptochrome végétal", mechanism: "Test RPM direct" },
      { authors: "Xu et al.", year: "2014", finding: "Champ statique de 500 µT améliore la phosphorylation du CRY chez Arabidopsis", mechanism: "Activation CRY" },
      { authors: "Xu et al.", year: "2015", finding: "Champ magnétique quasi nul supprime la floraison via la voie CRY", mechanism: "Floraison CRY-dépendante" },
      { authors: "Agliassa et al.", year: "2018", finding: "Le champ géomagnétique module la signalisation CRY-dépendante chez Arabidopsis", mechanism: "Signalisation CRY" },
      { authors: "PMC10005510", year: "2023", finding: "Les fréquences Wi-Fi (2,4 GHz) accélèrent la floraison de la laitue", mechanism: "RF → floraison" },
      { authors: "Ecological Indicators", year: "2023", finding: "L'exposition RF 868 MHz affecte la croissance et la reproduction de 10 espèces végétales sauvages", mechanism: "RF → reproduction" },
      { authors: "Haggerty", year: "2010", finding: "Le fond RF des antennes-relais associé au déclin des semis de peuplier faux-tremble", mechanism: "RF → croissance" },
    ],
    plantCryB2Title: "L'asymétrie B2/FAD : pourquoi les plantes sont un test plus propre",
    plantCryB2Plants: "Les PLANTES synthétisent leur propre B2 → l'approvisionnement en FAD est endogène → la fonction CRY dépend UNIQUEMENT de la perturbation RF → l'effet « relativement mineur » d'Ahmad 2020 est un test RPM PUR",
    plantCryB2Animals: "Les ANIMAUX nécessitent une B2 alimentaire → la fonction CRY dépend à la fois du RF ET du statut B2 → DOUBLE vulnérabilité (le RF perturbe les paires radicalaires + la carence en B2 épuise le cofacteur)",
    mastingTitle: "Le masting : Reproduction synchronisée sur 2 000 km",
    mastingIntro: "Le masting — la production massive synchronisée de graines par les arbres forestiers — est l'un des phénomènes de coordination les plus frappants de l'écologie. Des arbres à des distances continentales produisent des récoltes de graines exceptionnelles la même année, puis presque zéro les autres. Le signal de synchronisation doit être détectable de manière identique sur plus de 2 000 km.",
    mastingBermExplanation: "CRY2 lit la photopériode, la température ET le champ géomagnétique simultanément. De ces trois signaux, seul le champ géomagnétique est effectivement identique à l'échelle de 2 000 km — la photopériode varie avec la latitude, la température varie localement. BERM prédit que la composante géomagnétique de la synchronisation médiée par CRY2 est perturbée par le fond RF croissant, affaiblissant la cohérence du masting.",
    mastingResearchLabel: "Preuves de recherche sur le masting",
    mastingStudies: [
      { authors: "Nature Plants", year: "2024", finding: "Le solstice d'été agit comme un 'coup de pistolet céleste' — le signal de photopériode déclenche un masting synchronisé sur 2 000 km chez le hêtre européen", mechanism: "Signal de synchronisation" },
      { authors: "Bogdziewicz et al.", year: "2021", finding: "La synchronie du masting s'affaiblit dans les forêts européennes — attribuée au changement climatique, mais alternative BERM : la croissance du fond RF perturbe la détection géomagnétique du CRY2", mechanism: "Perte de synchronie" },
  ...
  ko: {
    ...
    catsperP1: "CatSper는 전압, pH, 온도 및 프로게스테론으로 조절된다. BERM의 EMF → CatSper 단계는 지정된 필드 프로토콜의 직접 데이터가 있을 때까지 미해결이다.",
    catsperP2: "CATSPER2 결손과 NNC55-0396은 필요성과 개입 지점을 확립하지만 차단제와 EMF 노출의 동등성을 확립하지 않는다 ([[ref:catsper_human|JCI 2024]]; [[ref:pmc6104424_nnc|Rennhack et al. 2018]]).",
    ...
    sentinelGradient: "정량 가능한 생식 감소율을 가진 7종에 걸쳐, 감소율은 EMF 노출 점수와 r = 0.84로 상관한다. 개 ([[ref:lea2016|Lea et al. 2016]]): 가정의 EMF 노출을 공유하는 영국 종견에서 26년간 정자 −1.0%/년 감소. 말 ([[ref:harris2023|Harris et al. 2023]]): 종마 정자 35년간 −0.75%/년 감소. 패턴은 꿀벌, 조류, 개구리 및 수생 종으로 확장 — 각각 전자기 환경에 비례하는 타임라인으로 감소.",
    sentinelTLink: "종간 추세는 비교 실험의 근거가 되지만 화학물질, 식이, 질병과 번식 이력도 고려해야 합니다. 추세만으로 EMF 원인이나 T–TFR 지연을 정할 수 없습니다.",
    sentinelGradientStat: "r = 0.84",
    ...
    theraBionicTitle: "프로토콜별 임상 앵커: TheraBionic",
    theraBionicLead: "하나의 반송파 × 포락선 × 채널 × 장기 경로를 제한하지만 환경·생식 전이를 검증하지 않는다.",
    theraBionicBody: "TheraBionic P1은 2023년 9월 26일 FDA HDE H220001 승인을 받은 27.12 MHz 진폭 변조 장치다.",
    theraBionicMechanism: "2019년 HCC 연구는 지정 프로토콜에서 Cav3.2 의존성 Ca²⁺ 유입을 보고했다. 이는 구성요소 근거이지 보편적 BERM 조직 커널의 증명이 아니다.",
    theraBionicSAR: "낮은 SAR 프로토콜의 생물활성은 이 시스템에 한정되며 SAR 비율만으로 다른 파형·장기·집단에 용량이나 부호를 이전할 수 없다.",
    theraBionicCCB: "차단제 관련 FDA 라벨은 경로와 일치하는 프로토콜 제약이지 무작위 차단제 실험은 아니다.",
    theraBionicImplication: "환경 노출, 생식 조직 및 인구 결과에는 별도의 전이·보정 검사가 필요하다.",
    theraBionicSurvival: "14/41에서 6개월 이상 안정 질환",
    theraBionicDevice: "27.12 MHz AM-RF, 종양 특이적 주파수",
    theraBionicChannel: "Cav3.2 (CACNA1H) T형 VGCC",
    theraBionicLevel: "직접 구성요소 — FDA HDE 2023 + 동료심사 HCC 기전",
    ...
    metabStudies: [
      ...
      { referenceId: "bhatt2012_glp1", authors: "Selway et al.", year: 2012, journal: "PLoS ONE", finding: "GLP-1이 L형 VGCC Ca²⁺ 미세도메인을 통해 β세포에서 ERK 활성화", mechanism: "3: 인슐린", level: "E" },
    ...
    cry2PathwayNote: "CRY2의 하류 효과는 일주기 시계를 넘어 확장된다. Iversen et al. (2025)은 CRY2가 TRP 계열 양이온 채널인 TRPC1과 물리적으로 상호작용하며, 이 복합체가 PEMF 노출 후 함께 핵으로 이동함을 보여주었다. 이 칼슘 유입 경로는 CRY2 의존적이고 (CRY2 침묵에 의해 차단), 광 의존적이며 (암소에서 소실), FAD 의존적이다 (RFK 침묵에 의해 감쇠) — 모두 RPM 메커니즘의 특징이다. 중요하게도, TRPC1은 전위 의존성 칼슘 채널이 아니며 L형 VGCC 차단제로 차단되지 않는다. 이는 경로 A와 B가 약리학적으로 분리 가능하게 유지됨을 의미하지만, 경로 B의 생물학적 범위는 이전에 가정된 것보다 크다.",
    ...
    solarNorthernTraits: [
      "홍채 색소침착 → 측정된 망막 광량 → χ_광학(입력은 투과 배율이 아니라 광량 자체)",
...
const SUB_PAGES = [
  ...
  },
  {
    slug: "pharmacology",
    icon: Pill,
    en: { title: "Pharmacological Evidence", desc: "Intervention profiles distinguish channel, calcium, clock and repair processes under defined protocols. Clinical effects and field-specific contrasts retain separate evidence." },
    fi: { title: "Farmakologinen näyttö", desc: "Interventioprofiilit erottelevat kanavia, kalsiumia, kelloa ja korjausta määritellyissä protokollissa. Kliiniset vaikutukset ja kenttäkohtaiset vertailut säilyttävät oman näyttönsä." },
    ja: { title: "薬理学的エビデンス", desc: "定義した介入でチャネル、Ca、時計、修復を区別する。臨床効果と電磁場固有の比較は別の証拠として保つ。" },
    fr: { title: "Preuves pharmacologiques", desc: "Les profils distinguent canaux, calcium, horloge et réparation dans des protocoles définis. Effets cliniques et contrastes propres au champ restent distincts." },
    ko: { title: "약리학적 근거", desc: "정의된 프로토콜에서 채널·칼슘·시계·복구를 구별한다. 임상 효과와 장 특이 비교는 별도 증거로 유지한다." },
  },
  {
  ...
  },
  {
    slug: "unbroken-chain",
    icon: Link2,
    en: { title: "Evidence Chain: Photon → Population", desc: "A cross-scale audit separating direct component evidence, composed convergence and open bridges. The proposed feedback network is testable; it is not one fully verified photon-to-population mechanism." },
    fi: { title: "Evidenssiketju: Fotoni → Populaatio", desc: "Skaalojen välinen auditointi erottaa suoran komponenttinäytön, koostetun konvergenssin ja avoimet sillat. Ehdotettu takaisinkytkentäverkko on testattava, ei yksi kokonaan todennettu mekanismi." },
    ja: { title: "証拠連鎖：光子→集団", desc: "直接構成要素証拠、構成された収束、未解決の橋渡しを分けるスケール横断監査。提案フィードバック網は検証可能だが、完全に検証済みの単一機構ではない。" },
    fr: { title: "Chaîne de preuves : Photon → Population", desc: "Audit inter-échelles distinguant preuve directe des composants, convergence composée et ponts ouverts. Le réseau de rétroaction proposé est testable, mais n'est pas un mécanisme entièrement vérifié." },
  ...
  },
  {
    slug: "testosterone",
    icon: TrendingDown,
    en: { title: "Testosterone: timing and the reproductive cascade", desc: "Documented testosterone decline precedes later national TFR decline episodes in Finland, the US MMAS comparison and Israel. New hormone, behavior and fertility studies constrain the cascade; a common causal lag remains unestimated." },
    fi: { title: "Testosteroni: ajoitus ja lisääntymiskaskadi", desc: "Dokumentoitu testosteronilasku edeltää myöhempää kansallista TFR-laskuvaihetta Suomessa, USA:n MMAS-vertailussa ja Israelissa. Uudet hormoni-, käyttäytymis- ja hedelmällisyystutkimukset tarkentavat kaskadia; yhteistä kausaalista viivettä ei ole estimoitu." },
    ja: { title: "テストステロン：時間的順序と生殖カスケード", desc: "フィンランド、米国MMAS、イスラエルでT低下が後のTFR低下に先行。ホルモン・行動・受胎研究がカスケードを制約します。共通の因果的遅延は未推定です。" },
    fr: { title: "Testostérone : chronologie et cascade reproductive", desc: "La baisse de T précède des baisses ultérieures du TFR en Finlande, dans la comparaison MMAS et en Israël. Les études hormonales, comportementales et de fécondité précisent la cascade; le délai causal commun reste non estimé." },
    ko: { title: "테스토스테론: 시간적 순서와 생식 연쇄", desc: "핀란드, 미국 MMAS 비교, 이스라엘에서 T 감소가 후속 TFR 감소에 선행합니다. 호르몬·행동·수태 연구가 연쇄를 제약하며 공통 인과적 지연은 아직 추정되지 않았습니다." },
  },
  {
    slug: "superposition",
    icon: Layers,
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      ...
      <section className="mb-14 max-w-4xl rounded-xl border border-card-border bg-card-bg p-6">
        ...
        <Link href={`/${locale}/civilization/epistapege#cognitive-immunology`} className="mt-4 inline-flex text-sm font-medium text-accent hover:underline">
        ...
        <div className="max-w-4xl mb-6">
          <svg viewBox="0 0 600 170" className="w-full" role="img" aria-label={d.metabMatrixLabel}>
            ...
            {[
              ...
              { name: "Selway", cells: ["full","empty","empty","full","empty","empty"] },
