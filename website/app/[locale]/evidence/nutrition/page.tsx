...
const COPY = {
  en: {
    ...
    section1: [
      ...
      "Näyttö tästä ravitsemuksellisesta portista tulee useista itsenäisistä tutkimuslinjoista. [[ref:hirano2017|Hirano ym. (2017)]] osoittivat, että FAD suoraan stabiloi CRY-proteiineja hiiren maksassa. Sherrard-laboratorio ([[ref:iversen2025|Iversen ym. 2025]]) osoitti, että solun FAD:n poistaminen poistaa magneettikentän suuntaerottelukyvyn täysin. [[ref:lamia2009|Lamia ym. (2009)]] paljastivat, että paastosensori AMPK aktiivisesti hajottaa CRY1:n, luoden ilmeisen paradoksin. Ja [[ref:majewska2025|Majewska ym. (2025)]] osoittivat, että CRY:n orientaatio kalvoilla — välttämätön suuntakohtaiselle aistimukselle — riippuu lipidikoostumuksesta.",
    ...
    epistemicText: "Tämä sivu esittää testattavan hypoteesin (L*-taso). Yksittäiset biokemialliset mekanismit ovat kokeellisesti vahvistettuja (E-taso): FAD stabiloi CRY:tä ([[ref:hirano2017|Hirano 2017]]), FAD vaaditaan magneettiseen herkkyyteen ([[ref:iversen2025|Iversen 2025]]), AMPK hajottaa CRY1:n ([[ref:lamia2009|Lamia 2009]]), CRY:n orientaatio riippuu kalvokoostumuksesta ([[ref:majewska2025|Majewska 2025]]). Synteesi — että ravitsemustila systemaattisesti säätelee polku B:n tehokkuutta väestötasolla — ei ole suoraan testattu. Kiinan B2-korrelaatio on ekologinen, ei kausaalinen. Tämä koskee yhtä lailla konventionaalisia selityksiä.",
    seeAlso: "Katso myös",
  ...
  ja: {
    ...
    section1: [
      ...
      "この栄養の門に関するエビデンスは複数の独立した研究ラインから得られている。[[ref:hirano2017|Hirano et al. (2017)]] はFADがマウス肝臓でCRYタンパク質を直接安定化することを示した。Sherrardの研究室 ([[ref:iversen2025|Iversen et al. 2025]]) は細胞のFADを枯渇させると磁場の方向選択性が完全に消失することを実証した。[[ref:lamia2009|Lamia et al. (2009)]] は断食センサーAMPKがCRY1を能動的に分解し見かけ上のパラドックスを生むことを明らかにした。そして[[ref:majewska2025|Majewska et al. (2025)]] は方向感知に不可欠な膜上でのCRYの配向が脂質組成に依存することを示した。",
    ...
    epistemicText: "このページはテスト可能な仮説（L*レベル）を提示する。個々の生化学的メカニズムは実験的に確認されている（Eレベル）：FADはCRYを安定化する ([[ref:hirano2017|Hirano 2017]])、FADは磁気感受性に必要である ([[ref:iversen2025|Iversen 2025]])、AMPKはCRY1を分解する ([[ref:lamia2009|Lamia 2009]])、CRYの配向は膜組成に依存する ([[ref:majewska2025|Majewska 2025]])。栄養状態が集団レベルで経路Bの有効性を体系的に調節するという統合は直接テストされていない。中国のB2相関は生態学的であり、因果的ではない。これは従来の説明にも同様に当てはまる。",
    seeAlso: "関連項目",
    eyesLink: "目の色と磁気受容 →",
  ...
  fr: {
    ...
    epistemicText: "Cette page presente une hypothese testable (niveau L*). Les mecanismes biochimiques individuels sont confirmes experimentalement (niveau E) : le FAD stabilise CRY ([[ref:hirano2017|Hirano 2017]]), le FAD est necessaire a la sensibilite magnetique ([[ref:iversen2025|Iversen 2025]]), AMPK degrade CRY1 ([[ref:lamia2009|Lamia 2009]]), l'orientation de CRY depend de la composition membranaire ([[ref:majewska2025|Majewska 2025]]). La synthese — selon laquelle l'etat nutritionnel module systematiquement l'efficacite de la voie B au niveau de la population — n'a pas ete directement testee. La correlation chinoise avec la B2 est ecologique, pas causale. Cela s'applique egalement aux explications conventionnelles.",
    seeAlso: "Voir aussi",
  ...
  ko: {
    ...
    section5: [
      "중국은 두드러진 생태학적 상관관계를 보여준다. 중국 국민건강조사(CNHS 2015-2017)는 인구 전반에서 90% 이상의 B2 부족을 발견했다. 중국은 동시에 세계에서 가장 가파른 TFR 감소를 보인다. 이는 생태학적 상관관계이지 인과적 증거는 아니지만, 메커니즘은 명확하다: B2 → FAD → CRY 안정성 → 경로 B 기능. 이는 기존 설명에도 동일하게 적용된다.",
    ...
    epistemicText: "이 페이지는 테스트 가능한 가설 (L* 수준)을 제시한다. 개별 생화학적 메커니즘은 실험적으로 확인되었다(E 수준): FAD는 CRY를 안정화하고 ([[ref:hirano2017|Hirano 2017]]), FAD는 자기 민감성에 필요하며 ([[ref:iversen2025|Iversen 2025]]), AMPK는 CRY1을 분해하고 ([[ref:lamia2009|Lamia 2009]]), CRY 배향은 막 조성에 의존한다 ([[ref:majewska2025|Majewska 2025]]). 영양 상태가 인구 수준에서 경로 B의 효과를 체계적으로 조절한다는 종합은 직접 테스트되지 않았다. 중국 B2 상관관계는 생태학적이며 인과적이지 않다. 이는 기존 설명에도 동일하게 적용된다.",
    seeAlso: "관련 항목",
    eyesLink: "눈 색깔과 자기수용 →",
