...
const COPY = {
  en: {
    ...
    section1: [
      ...
      "Evidenssi tästä ravitsemuksellisesta portista tulee useista itsenäisistä tutkimuslinjoista. [[ref:hirano2017|Hirano ym. (2017)]] osoittivat, että FAD suoraan stabiloi CRY-proteiineja hiiren maksassa. Sherrard-laboratorio ([[ref:yap2025|Yap ym. 2025]]) osoitti, että solun FAD:n poistaminen eliminoi magneettikentän suuntaerottelukyvyn täysin. [[ref:lamia2009|Lamia ym. (2009)]] paljastivat, että paastosensori AMPK aktiivisesti hajottaa CRY1:n, luoden ilmeisen paradoksin. Ja [[ref:majewska2025|Majewska ym. (2025)]] osoittivat, että CRY:n orientaatio kalvoilla — välttämätön suuntakohtaiselle aistimukselle — riippuu lipidikoostumuksesta.",
    ...
    section4: [
      "Evidenssissä on ilmeinen ristiriita. AMPK, solun ravintosensori, joka aktivoituu paastossa, fosforyloi CRY1:n ja kohdistaa sen hajotettavaksi ([[ref:lamia2009|Lamia ym. 2009, Science]]). Kuitenkin ainoa ihmisen magnetoreseptiota osoittanut tutkimus käytti nälkiintyneitä koehenkilöitä ([[ref:chae2019|Chae 2019]]). Jos paasto tuhoaa juuri sen proteiinin, joka aistii magneettikenttiä, miten paasto voi tehostaa magnetoreseptiota?",
    ...
    section5: [
      ...
      "[[ref:wacker2000|Wacker ym. (2000)]] havaitsivat, että B2-puutteisilla äideillä oli 4,7-kertainen pre-eklampsian riski (OR 4,7, CI 1,8-12,2). IVF-klinikoiden data osoittaa B2-lisän parantavan alkionlaatumittareita. Nämä ovat itsenäisiä evidenssilinjoja jotka konvergoivat samaan biokemialliseen pullonkaulaan.",
    ...
    epistemicText: "Tämä sivu esittää testattavan hypoteesin (L*-taso). Yksittäiset biokemialliset mekanismit ovat kokeellisesti vahvistettuja (E-taso): FAD stabiloi CRY:tä ([[ref:hirano2017|Hirano 2017]]), FAD vaaditaan magneettiseen herkkyyteen ([[ref:yap2025|Yap 2025]]), AMPK hajottaa CRY1:n ([[ref:lamia2009|Lamia 2009]]), CRY:n orientaatio riippuu kalvokoostumuksesta ([[ref:majewska2025|Majewska 2025]]). Synteesi — että ravitsemustila systemaattisesti moduloi polku C:n tehokkuutta väestötasolla — ei ole suoraan testattu. Kiinan B2-korrelaatio on ekologinen, ei kausaalinen.",
    seeAlso: "Katso myös",
  ...
  ja: {
    ...
    section1: [
      ...
      "これらの発見を合わせると、経路Cの3つの栄養制御点が定義される：(1) chromophore装填のためのB2/FAD利用可能性、(2) CRY配向のための膜脂質組成、(3) 断食時のAMPK媒介CRY代謝回転速度。",
    ],
    ...
    section5: [
      "中国は顕著な生態学的相関を示す。中国国民健康調査（CNHS 2015-2017）は、人口全体で90%超のB2不足を認めた。中国では同時に世界で最も急なTFR低下が起きている。これは生態学的相関であって因果的証明ではないが、メカニズムは明確である：B2 → FAD → CRY安定性 → 経路Cの機能。",
    ...
    epistemicText: "このページはテスト可能な仮説（L*レベル）を提示する。個々の生化学的メカニズムは実験的に確認されている（Eレベル）：FADはCRYを安定化する ([[ref:hirano2017|Hirano 2017]])、FADは磁気感受性に必要である ([[ref:yap2025|Yap 2025]])、AMPKはCRY1を分解する ([[ref:lamia2009|Lamia 2009]])、CRYの配向は膜組成に依存する ([[ref:majewska2025|Majewska 2025]])。栄養状態が集団レベルで経路Cの有効性を体系的に調節するという統合は直接テストされていない。中国のB2相関は生態学的であり、因果的ではない。",
    seeAlso: "関連項目",
    eyesLink: "目の色と磁気受容 →",
  ...
  fr: {
    ...
    section1: [
      ...
      "Ensemble, ces resultats definissent trois points de controle nutritionnel pour la voie C : (1) la disponibilite en B2/FAD pour le chargement du chromophore, (2) la composition lipidique membranaire pour l'orientation de CRY, et (3) le taux de renouvellement de CRY medie par AMPK pendant le jeune.",
    ],
    ...
    epistemicText: "Cette page presente une hypothese testable (niveau L*). Les mecanismes biochimiques individuels sont confirmes experimentalement (niveau E) : le FAD stabilise CRY ([[ref:hirano2017|Hirano 2017]]), le FAD est necessaire a la sensibilite magnetique ([[ref:yap2025|Yap 2025]]), AMPK degrade CRY1 ([[ref:lamia2009|Lamia 2009]]), l'orientation de CRY depend de la composition membranaire ([[ref:majewska2025|Majewska 2025]]). La synthese — selon laquelle l'etat nutritionnel module systematiquement l'efficacite de la voie C au niveau de la population — n'a pas ete directement testee. La correlation chinoise avec la B2 est ecologique, pas causale.",
    seeAlso: "Voir aussi",
  ...
  ko: {
    ...
    section1: [
      ...
      "이러한 발견들을 종합하면 경로 C의 3가지 영양 제어점이 정의된다: (1) chromophore 장전을 위한 B2/FAD 가용성, (2) CRY 배향을 위한 막 지질 조성, (3) 단식 중 AMPK 매개 CRY 대사회전 속도.",
    ],
    section2Title: "주요 증거",
    ...
    section5: [
      "중국은 두드러진 생태학적 상관관계를 보여준다. 중국 국민건강조사(CNHS 2015-2017)는 인구 전반에서 90% 이상의 B2 부족을 발견했다. 중국은 동시에 세계에서 가장 가파른 TFR 감소를 보인다. 이는 생태학적 상관관계이지 인과적 증거는 아니지만, 메커니즘은 명확하다: B2 → FAD → CRY 안정성 → 경로 C 기능.",
    ...
    epistemicText: "이 페이지는 테스트 가능한 가설 (L* 수준)을 제시한다. 개별 생화학적 메커니즘은 실험적으로 확인되었다(E 수준): FAD는 CRY를 안정화하고 ([[ref:hirano2017|Hirano 2017]]), FAD는 자기 민감성에 필요하며 ([[ref:yap2025|Yap 2025]]), AMPK는 CRY1을 분해하고 ([[ref:lamia2009|Lamia 2009]]), CRY 배향은 막 조성에 의존한다 ([[ref:majewska2025|Majewska 2025]]). 영양 상태가 인구 수준에서 경로 C의 효과를 체계적으로 조절한다는 종합은 직접 테스트되지 않았다. 중국 B2 상관관계는 생태학적이며 인과적이지 않다.",
    seeAlso: "관련 항목",
    eyesLink: "눈 색깔과 자기수용 →",
