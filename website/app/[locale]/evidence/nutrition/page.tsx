...
const COPY = {
  en: {
    ...
    epistemicText: "Tämä sivu esittää testattavan hypoteesin (L*-taso). Yksittäiset biokemialliset mekanismit ovat kokeellisesti vahvistettuja (E-taso): FAD stabiloi CRY:tä ([[ref:hirano2017|Hirano 2017]]), FAD vaaditaan magneettiseen herkkyyteen ([[ref:yap2025|Yap 2025]]), AMPK hajottaa CRY1:n ([[ref:lamia2009|Lamia 2009]]), CRY:n orientaatio riippuu kalvokoostumuksesta ([[ref:majewska2025|Majewska 2025]]). Synteesi — että ravitsemustila systemaattisesti säätelee polku B:n tehokkuutta väestötasolla — ei ole suoraan testattu. Kiinan B2-korrelaatio on ekologinen, ei kausaalinen. Tämä koskee yhtä lailla konventionaalisia selityksiä.",
    seeAlso: "Katso myös",
  ...
  ja: {
    ...
    epistemicText: "このページはテスト可能な仮説（L*レベル）を提示する。個々の生化学的メカニズムは実験的に確認されている（Eレベル）：FADはCRYを安定化する ([[ref:hirano2017|Hirano 2017]])、FADは磁気感受性に必要である ([[ref:yap2025|Yap 2025]])、AMPKはCRY1を分解する ([[ref:lamia2009|Lamia 2009]])、CRYの配向は膜組成に依存する ([[ref:majewska2025|Majewska 2025]])。栄養状態が集団レベルで経路Bの有効性を体系的に調節するという統合は直接テストされていない。中国のB2相関は生態学的であり、因果的ではない。これは従来の説明にも同様に当てはまる。",
    seeAlso: "関連項目",
    eyesLink: "目の色と磁気受容 →",
  ...
  ko: {
    ...
    epistemicText: "이 페이지는 테스트 가능한 가설 (L* 수준)을 제시한다. 개별 생화학적 메커니즘은 실험적으로 확인되었다(E 수준): FAD는 CRY를 안정화하고 ([[ref:hirano2017|Hirano 2017]]), FAD는 자기 민감성에 필요하며 ([[ref:yap2025|Yap 2025]]), AMPK는 CRY1을 분해하고 ([[ref:lamia2009|Lamia 2009]]), CRY 배향은 막 조성에 의존한다 ([[ref:majewska2025|Majewska 2025]]). 영양 상태가 인구 수준에서 경로 B의 효과를 체계적으로 조절한다는 종합은 직접 테스트되지 않았다. 중국 B2 상관관계는 생태학적이며 인과적이지 않다. 이는 기존 설명에도 동일하게 적용된다.",
    seeAlso: "관련 항목",
    eyesLink: "눈 색깔과 자기수용 →",
