...
const COPY = {
  en: {
    ...
    narratives: [
      {
        ...
        paragraphs: [
          ...
          "Ensimmäinen 5G-taajuusspesifinen testisdata ([[ref:bektas2026|Bektas ym. 2026]], Bioelectromagnetics) tukee palautumisikkunakonseptia eri kulmasta: CoQ10-lisäravinto lievitti 3,5 GHz RF:n aiheuttamaa testis- ja oksidatiivista vauriota rotilla. CoQ10-interventio osoittaa mekanismin reversiibeliuden — oksidatiivinen polku on farmakologisesti estettävissä, yhdenmukainen mallin net_daily = damage_rate × t_emf × (1 − exp(−t_free / τ_repair)) kanssa. CoQ10 lisää antioksidanttikapasiteettia, lyhentäen efektiivisesti τ_repair-arvoa ja vähentäen nettovauriota. Tämä on palautumisikkunan farmakologinen analogi: altistusajan vähentämisen sijaan interventio tehostaa korjausnopeutta.",
        ],
        studies: [
      ...
      },
      {
        ...
        studies: [
          ...
          { citation: "Wang ym. (eNeuro)", year: 2019, referenceId: "wang2019_eneuro", note: "Aikaisempi EEG-evidenssi: alfa-ERD magneettikentän rotaation jälkeen suojatussa kammiossa." },
        ],
      },
    ],
    ...
    melatoninBridge: {
      ...
      references: [
        { citation: "Battelle / Wilson ym.", year: 1980, referenceId: "battelle1980_emf_melatonin", finding: "EMF suppressoi yöllistä melatoniinia koe-eläimissä (60 Hz, ELF)" },
        ...
        { citation: "Rad. Prot. Dosimetry", year: 2013, referenceId: "rad_prot_dosimetry2013_emf_melatonin", finding: "RF-EMF ja melatoniinisuppressio: epidemiologinen katsaus (tulokset epäjohdonmukaisia ihmisillä)" },
      ],
    ...
    seeAlso: {
      title: "Katso myös",
      links: [
        { title: "Evidenssirekisteri", description: "Kaikki evidenssitietueet ja -narratiivit" },
    ...
    narratives: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "この研究はBERM経路Cのラジカルペアメカニズムを直接支持する：ヒトのニューロンが地球強度レベル（~50 μT）の静的磁場変化に検出可能に反応するなら、CRY/RPMトランスダクション経路はヒトで神経学的に活性であることが実証的に確認される——渡り鳥だけでなく。アルファ脱同期パターンは、磁場検出がCRY1が青錐体外節に局在する網膜レベル（[[ref:bartolke2025|Bartölke 2025]]）だけでなく、皮質レベルで生じることを示唆する。",
    ...
    narratives: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Cela soutient directement la voie C de BERM (EMF → suppression pinéale de la mélatonine → perturbation de la pulsatilité de la GnRH → HPG → fonction gonadique). La magnitude de suppression de 20–50 % est biologiquement significative et cohérente avec la fonction v17_night_fraction() de BERM, où les EMF sont une composante du triple coup nocturne (mélanopsine + CRY + suppression de la mélatonine). Le fait que la magnitude de suppression soit inférieure à celle induite par la lumière (>90 %) est cohérent avec la modélisation par BERM des EMF comme une voie de perturbation nocturne parmi plusieurs, et non comme le seul facteur. Note méthodologique : seulement 27 % des études examinées ont satisfait à des normes méthodologiques élevées ; 48 % des études animales manquaient de contrôles sham adéquats. La transition des effets cellulaires à la perturbation circadienne systémique n'est pas entièrement établie cliniquement.",
    ...
    narratives: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "이는 BERM 경로 C(EMF → 송과체 멜라토닌 억제 → GnRH 박동성 장애 → HPG → 생식선 기능)를 직접 지지한다. 20–50%의 억제 규모는 생물학적으로 유의하며, EMF가 야간 삼중 타격(멜라놉신 + CRY + 멜라토닌 억제)의 한 구성 요소인 BERM의 v17_night_fraction() 함수와 일치한다. 억제 규모가 광 유발(>90%)보다 작다는 것은 BERM이 EMF를 복수의 야간 교란 경로 중 하나로 모델링하고 유일한 구동 요인으로 삼지 않는 것과 일치한다. 방법론적 주기: 검토된 연구의 27%만이 높은 방법론적 기준을 충족했다; 동물 연구의 48%가 적절한 거짓 대조를 결여했다. 세포 효과에서 전신성 개일 교란으로의 전환은 임상적으로 완전히 확립되지 않았다.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "이 연구는 BERM 경로 C의 래디컬 쌍 메커니즘을 직접 지지한다: 인간 뉴런이 지구 강도 수준(~50 μT)의 정적 자기장 변화에 감지 가능하게 반응한다면, CRY/RPM 전달 경로는 인간에서 신경학적으로 활성임이 실증적으로 확인된다 — 철새에서만이 아니다. 알파 비동기화 패턴은 자기장 감지가 CRY1이 청색 원뿔 외절에 국재화된 망막 수준([[ref:bartolke2025|Bartölke 2025]])에서뿐만 아니라 피질 수준에서 발생함을 시사한다.",
