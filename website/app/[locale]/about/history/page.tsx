...
const COPY = {
  en: {
    ...
    entries: [
      ...
      ["Lindgrenistä johdetut mittaushypoteesit", "Taustariippuvuus, vektorin orientaatio, paikallinen geometria, spektrisisältö ja ajoitus nostettiin nimenomaisiksi kokeellisiksi muuttujiksi. Ne säilyvät testattavina edeltävinä premisseinä, eivät suoraan ihmisen lisääntymisriskin tai TFR:n arviona."],
  ...
  ja: {
    ...
    entries: [
      ...
      ["Lindgrenに由来する測定仮説", "背景依存性、ベクトル配向、局所的幾何構造、スペクトル成分、およびタイミングが、明示的な実験変数として設定された。これらはヒトの生殖リスクやTFRの直接推定値としてではなく、検証可能な上流前提として保持されている。"],
      ["BERM v17", "現行の経路は、FieldState、臓器局所的伝達、可逆的/持続的臓器状態、カップル受胎能、ASFR、およびTFRをそれぞれ独立した変数として維持する。携帯電話契約密度は、電磁環境全体の複合的代理指標として機能する。"],
  ...
  fr: {
    ...
    entries: [
      ...
      ["Hypothèses de mesure dérivées de Lindgren", "La dépendance au bruit de fond, l'orientation vectorielle, la géométrie locale, le contenu spectral et la synchronisation sont devenus des variables expérimentales explicites. Ils sont conservés comme prémisses amont vérifiables, et non comme estimation directe du risque reproductif humain ou du TFR."],
      ["BERM v17", "La voie active maintient désormais FieldState, le transfert local par organe, l'état organique réversible/persistant, la capacité du couple, l'ASFR et le TFR comme variables distinctes. La densité d'abonnements mobiles sert d'indicateur composite pour l'environnement électromagnétique global."],
  ...
  ko: {
    ...
    entries: [
      ...
      ["Lindgren 유래 측정 가설", "배경 의존성, 벡터 방향, 국소 기하학, 스펙트럼 구성, 타이밍이 명시적 실험 변수로 설정되었다. 이들은 인간 생식 위험이나 TFR의 직접적 추정치가 아닌, 검증 가능한 상위 전제로서 유지된다."],
      ["BERM v17", "현행 경로는 FieldState, 장기 국소 전달, 가역적/지속적 장기 상태, 부부 수태능, ASFR, TFR을 각각 별개의 변수로 유지한다. 이동통신 가입 밀도는 전체 전자기 환경에 대한 복합 대리 지표로 기능한다."],
