...
const t: Record<string, Copy> = {
  ...
  ja: {
    ...
    deepHistoryText: [
      ...
      "BERMはこれをNorthern Package、すなわち集団レベルの候補生物学的修飾因子の仮説的プロファイルとして定式化します。分子・光学・細胞・環境・集団の各尺度で反復されるχ表記はモデリング上の略記であり、Lindgren幾何学から導出された単一の生物学的選択則ではありません。電化がエンドポイント固有の応答カーネルを介して集団差を顕在化させるという仮説であり、符号・尺度・校正は未解決です。",
  ...
  ko: {
    ...
    deepHistoryText: [
      ...
      "BERM은 이를 Northern Package, 즉 인구 수준의 후보 생물학적 조절인자에 대한 가설적 프로파일로 정식화합니다. 분자·광학·세포·환경·인구 규모에서 반복되는 χ 표기는 모델링 약어이며 Lindgren 기하학에서 도출된 하나의 생물학적 선택 규칙이 아닙니다. 전기화가 종점별 반응 커널을 통해 인구 이질성을 드러낸다는 가설이며 부호·척도·보정은 미해결입니다.",
