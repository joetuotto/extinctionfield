...
const t: Record<string, Copy> = {
  en: {
    ...
    deepHistoryText: [
      ...
      "BERM formalises this as the Northern Package: an imported population-level biological susceptibility profile χ_pop. Its molecular, optical, cellular, environmental and population components are separately named candidate functions (such as χ_optical and χ_molecular), not repeated biological uses of the L1 geometric coefficient χ_geo. Connecting either class to an exposure observable remains an open L0→L2 step.",
      "This framework generates a falsifiable L3 hypothesis: populations with high candidate χ_pop profiles (Scandinavian-origin) should show steeper TFR decline under an equivalent measured protocol than populations with low profiles (Sub-Saharan African-origin). The Amish–Mennonite gradient and the COVID work-from-home baby bump are proposed natural experiments, not validations of χ_geo.",
    ],
    deepHistoryNote: "The Northern Package hypothesis is rated L* (novel, falsifiable). OCA2 + LCT co-selection is established (E-level); the cryptochrome interpretation is the novel, testable extension.",
    ...
    deepHistoryText: [
      ...
      "BERM formalisoi tämän Pohjoisena pakettina: tuotuna populaatiotason biologisena herkkyysprofiilina χ_pop. Sen molekyyli-, optinen, solu-, ympäristö- ja populaatiokomponentit ovat erikseen nimettyjä ehdokasfunktioita (kuten χ_optical ja χ_molecular), eivät L1-geometriakertoimen χ_geo toistuvia biologisia käyttötapoja. Kummankin luokan liittäminen altistushavaintoon on avoin L0→L2-askel.",
      "Viitekehys tuottaa falsifioitavan L3-hypoteesin: korkean ehdokas-χ_pop-profiilin populaatioiden (skandinaavisperäiset) pitäisi osoittaa jyrkempää TFR-laskua samanarvoisessa mitatussa protokollassa kuin matalan profiilin populaatioiden (Saharan eteläpuoleiset afrikkalaisperäiset). Amish–mennoniittigradientti ja COVID-etätyön baby bump ovat ehdotettuja luonnollisia kokeita, eivät χ_geo:n validointeja.",
    ],
    deepHistoryNote: "Pohjoinen paketti -hypoteesi on arvioitu tasolle L* (deduktiivinen, falsifioitava ehdokas). OCA2 + LCT -koselektio on vakiintunut (E-taso); kryptokromitulkinta on uusi, testattava laajennus.",
  ...
  ja: {
    ...
    deepHistoryText: [
      ...
      "BERMはこれをNorthern Package、すなわち導入された集団レベルの生物学的感受性プロファイルχ_popとして定式化する。分子・光学・細胞・環境・集団の各成分はχ_opticalやχ_molecularのように別名を持つ候補関数であり、L1幾何係数χ_geoを生物学へ反復適用したものではない。どちらのクラスも曝露観測量へ結ぶL0→L2段階は未解決である。",
      "この枠組みは反証可能なL3仮説を生成する：候補χ_popが高い集団（スカンジナビア起源）は、同等の測定プロトコル下で低い集団（サハラ以南アフリカ起源）より急なTFR低下を示すはずである。Amish–Mennonite勾配とCOVID在宅勤務ベビーブームは提案された自然実験であり、χ_geoの検証ではない。",
    ],
  ...
  fr: {
    ...
    deepHistoryText: [
      ...
      "BERM formalise ceci comme le Northern Package : un profil biologique populationnel importé χ_pop. Ses composantes moléculaire, optique, cellulaire, environnementale et populationnelle sont des fonctions candidates nommées séparément (comme χ_optical et χ_molecular), non des applications biologiques répétées du coefficient géométrique L1 χ_geo. Le lien de chaque classe à un observable d’exposition reste une étape L0→L2 ouverte.",
      "Ce cadre génère une hypothèse L3 falsifiable : les populations à profil candidat χ_pop élevé (d’origine scandinave) devraient présenter un déclin de TFR plus prononcé sous un protocole mesuré équivalent que les populations à profil faible (d’origine subsaharienne africaine). Le gradient Amish–Mennonite et le baby-boom du télétravail COVID sont des expériences naturelles proposées, pas des validations de χ_geo.",
    ],
  ...
  ko: {
    ...
    deepHistoryText: [
      ...
      "BERM은 이를 Northern Package, 즉 도입된 인구 수준 생물학적 감수성 프로파일 χ_pop으로 공식화한다. 분자·광학·세포·환경·인구 구성요소는 χ_optical 및 χ_molecular처럼 별도로 명명된 후보 함수이며, L1 기하 계수 χ_geo를 생물학에 반복 적용한 것이 아니다. 두 부류를 노출 관측량에 연결하는 L0→L2 단계는 열려 있다.",
      "이 프레임워크는 반증 가능한 L3 가설을 만든다. 후보 χ_pop이 높은 인구(스칸디나비아 기원)는 동등한 측정 프로토콜에서 낮은 인구(사하라 이남 아프리카 기원)보다 더 가파른 TFR 감소를 보여야 한다. Amish–Mennonite 기울기와 COVID 재택근무 베이비 붐은 제안된 자연실험이지 χ_geo의 검증이 아니다.",
    ],
    deepHistoryNote: "Northern Package 가설은 L*(연역적이며 반증 가능한 후보)로 평가됩니다. OCA2 + LCT 공동 선택은 확립되었습니다(E 수준); 크립토크롬 해석은 새로운 검증 가능한 확장입니다.",
