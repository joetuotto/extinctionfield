...
const COPY = {
  en: {
    ...
    receiver: "BERM names the bridge instead of hiding it inside a mechanism label. Let zᵣ be a measurable input to a particular receptor process and Kᵣ its receiving operator. The state Sᵣ includes the properties that make the receiver responsive: orientation, cofactors, redox state, biological phase and recovery history.",
    ...
    bridge: "The L2 geometry-to-receptor coupling remains open: Lindgren’s metric relation does not itself supply K. BERM’s biological continuation is conditional on this named coupling. Component experiments can locate and constrain the receiver, its state variables and the later transitions, while the physical bridge retains its own measurable task.",
    ...
    receiver: "BERM nimeää kytkennän näkyviin. Olkoon zᵣ tietyn vastaanottavan prosessin mitattava syöte ja Kᵣ sen vastaanotto-operaattori. Tila Sᵣ sisältää vastaanottavuuteen vaikuttavat ominaisuudet: orientaation, kofaktorit, redox-tilan, biologisen vaiheen ja palautumishistorian.",
    ...
    bridge: "Geometriasta vastaanottimeen kulkeva L2-kytkentä on avoin: Lindgrenin metriikkasuhde ei itsessään anna K:ta. BERM jatkaa biologista ketjua tämän nimetyn kytkennän ehdolla. Osakokeet paikantavat ja rajaavat vastaanotinta, sen tilamuuttujia ja myöhempiä siirtymiä samalla, kun fysikaaliselle liitokselle jää oma mitattava tehtävänsä.",
...
export default async function PhysicsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <ExplanationHub locale={locale} copy={COPY} {...d} icon={Atom} stage="physics">
    ...
    <ExplanationSection {...d.contents[2]}>{p(d.receiver)}<MathBlock tex={String.raw`z_r(t)=\int_0^\infty K_r^{\mu\nu}(\tau;S_r(t))\,\delta g_{\mu\nu}(t-\tau)\,d\tau`} />{p(d.receiverMeaning)}<div className="rounded-lg border border-accent/30 bg-accent/5 p-5"><h3 className="mb-3 text-base font-semibold">{d.bridgeTitle}</h3>{p(d.bridge)}</div></ExplanationSection>
