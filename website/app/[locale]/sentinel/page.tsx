...
import { DkcSentinelAllometry } from "@/components/DkcSentinelAllometry";
...
const COPY = {
  en: {
    ...
    whaleP3: "Three observational lines motivate a shared solar/geomagnetic timing hypothesis, but not a universal χ_geo response across endpoints. [[ref:geomag_263|Cardiovascular mortality associations in 263 US cities]] and [[ref:solar_birth_11yr|11-year birth-cycle associations across 9 regions]] have their own endpoint provenance. CRY/melatonin → HPG and cetacean magnetoreception are separately imported L3 candidates; neither is derived from χ_geo.",
    whaleM3Title: "Prediction M3: Effect-size temporal attenuation",
    whaleM3: "Candidate prediction M3 is that the Granger whale-stranding effect and 11-year birth-cycle amplitude attenuate as anthropogenic background rises. Testing it requires local natural and anthropogenic measurements z, an explicitly declared dimensionless normalization x=N(z), and endpoint-specific response models. It is a signal-masking hypothesis, not a consequence of the L1 χ_geo formula.",
    ...
    threeAxisP1: "Land, water and cross-species comparisons offer complementary tests of local reception and function. Their shared data and assumptions must be tracked before treating their evidence as independent.",
    ...
    threeAxisGradient: "Gradient axis: r=0.84 across seven groups with assumed burden scores; ecological context and endpoint differences remain",
    threeAxisP2: "A stronger comparison predicts species differences from measured local fields, declared receptor biology and functional outcomes. Convergence can constrain that model, but false-positive probabilities cannot be multiplied when datasets, selected groups or bridge assumptions overlap.",
    ...
    threeAxisP1: "Maa-, vesi- ja lajienväliset vertailut tarjoavat toisiaan täydentäviä paikallisen vastaanoton ja toiminnan testejä. Yhteiset aineistot ja oletukset on jäljitettävä ennen näyttöjen käsittelyä riippumattomina.",
  ...
  ja: {
    ...
    whaleP3: "3つの観察線は共通する太陽・地磁気タイミング仮説を動機づけるが、エンドポイントを横断する普遍的χ_geo応答を示さない。[[ref:geomag_263|263米国都市の心血管死亡関連]]と[[ref:solar_birth_11yr|9地域の11年出生周期関連]]は、それぞれ固有の来歴を保つ。CRY/メラトニン→HPGと鯨類の磁気受容は別個の導入L3候補であり、χ_geoから導出されない。",
    whaleM3Title: "予測M3：効果量の時間的減衰",
    whaleM3: "候補予測M3は、人為的背景の上昇に伴い、Grangerのクジラ座礁効果と11年出生周期振幅が減衰するというものだ。検定には局所的な自然・人為測定z、明示した無次元正規化x=N(z)、エンドポイント別応答モデルが必要である。これは信号マスキング仮説であり、L1 χ_geo式の帰結ではない。",
    whaleNote: "Granger 2020は太陽活動指数と座礁数の観察的相関[C]である。クジラ位置の曝露を測定せず、鯨類でCRY2を実証しない。引用されたCRY2証拠はショウジョウバエ由来である。正規化と幾何学–観測量の橋渡しはL0→L2で未解決、下流生物学は導入L3、M3は未検証である。",
  ...
  fr: {
    ...
    whaleP3: "Trois lignes observationnelles motivent une hypothèse commune de synchronisation solaire/géomagnétique, mais pas une réponse χ_geo universelle entre endpoints. [[ref:geomag_263|L'association avec la mortalité cardiovasculaire dans 263 villes américaines]] et [[ref:solar_birth_11yr|l'association du cycle natal de 11 ans dans 9 régions]] gardent chacune leur provenance. CRY/mélatonine → HPG et la magnétoréception des cétacés sont des candidats L3 importés séparés, non dérivés de χ_geo.",
    whaleM3Title: "Prédiction M3 : atténuation temporelle de la taille d'effet",
    whaleM3: "La prédiction candidate M3 veut que l'effet Granger sur les échouages et l'amplitude du cycle natal de 11 ans s'atténuent lorsque le fond anthropique augmente. Le test exige des mesures locales naturelles et anthropiques z, une normalisation sans dimension explicite x=N(z) et des modèles de réponse propres aux endpoints. C'est une hypothèse de masquage du signal, non une conséquence de la formule L1 χ_geo.",
    whaleNote: "Granger 2020 est une corrélation observationnelle [C] entre indices solaires et échouages. Il ne mesure pas l'exposition locale ni ne démontre CRY2 chez les cétacés ; la preuve CRY2 citée vient de la drosophile. La normalisation et le pont géométrie–observable restent ouverts en L0→L2, la biologie aval est importée en L3 et M3 n'est pas testée.",
  ...
  ko: {
    ...
    whaleP3: "세 관찰 선은 공통 태양·지자기 타이밍 가설을 동기화하지만 종점을 가로지르는 보편적 χ_geo 반응을 뜻하지 않는다. [[ref:geomag_263|263개 미국 도시의 심혈관 사망 연관]]과 [[ref:solar_birth_11yr|9개 지역의 11년 출생 주기 연관]]은 각자의 종점 출처를 유지한다. CRY/멜라토닌→HPG와 고래류 자기수용은 별도의 도입 L3 후보이며 χ_geo에서 도출되지 않는다.",
    whaleM3Title: "예측 M3: 효과 크기의 시간적 감쇠",
    whaleM3: "후보 예측 M3은 인위적 배경이 상승할수록 Granger 고래 좌초 효과와 11년 출생 주기 진폭이 감쇠한다는 것이다. 검정에는 지역의 자연·인위 측정 z, 명시적 무차원 정규화 x=N(z), 종점별 반응 모델이 필요하다. 이는 신호 마스킹 가설이지 L1 χ_geo 공식의 귀결이 아니다.",
    whaleNote: "Granger 2020은 태양 활동 지수와 좌초 수의 관찰 상관[C]이다. 고래 위치 노출을 측정하지 않고 고래류 CRY2를 입증하지 않는다. 인용된 CRY2 증거는 초파리에서 왔다. 정규화와 기하학–관측량 다리는 L0→L2에서 열려 있고 하류 생물학은 도입 L3이며 M3은 미검정이다.",
