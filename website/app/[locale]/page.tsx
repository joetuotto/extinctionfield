...
const COPY = {
  ...
  ja: {
    ...
    heroContext: "出生率は世界中で崩壊しています。精子数は半減しました。テストステロンは、体重が増えていない男性でさえ、測定されたすべての先進国集団で低下しています。ミツバチのコロニー、鳥類の個体数、両生類が同じタイムラインで減少しています。",
    ...
    bridgeP2: "しかしこれらの説明は基本的な検証に失敗します。すべての先進国でなぜこれらすべての傾向が同時に起きているのか、そしてなぜそれらが社会的・経済的変数よりも電気インフラとより強く相関するのかを説明できません。",
    ...
    paradoxText: "54カ国にわたり、住宅用電力消費量が出生率低下の最も強い予測因子です（RMSE 0.533）。携帯電話契約数 — 情報デバイス — は最も弱い予測因子です（RMSE 1.053）。インフラ変数が最もよく予測します — 物理的曝露メカニズムと一致しています。",
    paradoxStat1: "LOOCV RMSE",
    paradoxVal1: "0.522",
    paradoxStat2: "R²",
    paradoxVal2: "0.851",
    paradoxStat3: "0.5以内の国",
    paradoxVal3: "74%",
    paradoxCta: "横断的公式",
    ...
    epistemicNote: (n: number) => `BERM v17は反証可能な研究モデルであり、確実性ではありません。複数の研究分野にまたがる${n}件の登録済み資料。24以上の規制当局により検証された非熱メカニズム。横断的公式：LOOCV RMSE 0.522、74%の国が予測から0.5人以内。住宅用電力消費量はGDPをTFR予測因子として21%上回ります。日付と信頼区間を持つ固定予測。予測が失敗すれば、モデルは間違っています。`,
  ...
  fr: {
    ...
    bridgeP2: "Mais ces explications échouent à un test fondamental : elles ne peuvent pas expliquer pourquoi toutes ces tendances se produisent simultanément, dans tous les pays industrialisés, et pourquoi elles corrèlent plus fortement avec l'infrastructure électrique qu'avec n'importe quelle variable sociale ou économique.",
    ...
    paradoxText: "Dans 54 pays, la consommation résidentielle d'électricité est le prédicteur le plus fort du déclin de la fécondité (RMSE 0,533). Les abonnements de téléphonie mobile — le dispositif d'information — sont le plus faible (RMSE 1,053). La variable d'infrastructure prédit le mieux — cohérent avec un mécanisme d'exposition physique.",
    paradoxStat1: "LOOCV RMSE",
    paradoxVal1: "0,522",
    paradoxStat2: "R²",
    paradoxVal2: "0,851",
    paradoxStat3: "Pays dans la marge de 0,5",
    paradoxVal3: "74 %",
    ...
    epistemicNote: (n: number) => `BERM v17 est un modèle de recherche falsifiable, pas une certitude. ${n} sources enregistrées issues de plusieurs domaines de recherche. Plus de 24 mécanismes non thermiques validés réglementairement. Formule transversale : LOOCV RMSE 0,522, 74 % des pays à moins de 0,5 enfant de la prédiction. La consommation résidentielle d'électricité surpasse le PIB comme prédicteur du TFR de 21 %. Prédictions verrouillées avec dates et intervalles de confiance. Si les prédictions échouent, le modèle est faux.`,
  ...
  ko: {
    ...
    heroContext: "출산율이 전 세계적으로 붕괴하고 있습니다. 정자 수는 절반으로 줄었습니다. 테스토스테론은 체중이 증가하지 않은 남성에서도 측정된 모든 산업화 인구에서 감소하고 있습니다. 꿀벌 군집, 조류 개체수, 양서류가 같은 시간대에 감소하고 있습니다.",
    ...
    bridgeP2: "그러나 이러한 설명들은 기본적인 검증에 실패합니다: 왜 이 모든 추세가 동시에, 모든 산업화 국가에서 발생하고 있는지, 그리고 왜 이들이 사회적 또는 경제적 변수보다 전기 인프라와 더 강하게 상관되는지 설명할 수 없습니다.",
    ...
    paradoxText: "54개국에서 주거용 전력 소비가 출산율 감소의 가장 강력한 예측 인자입니다(RMSE 0.533). 휴대전화 가입 수 — 정보 기기 — 는 가장 약합니다(RMSE 1.053). 인프라 변수가 가장 잘 예측합니다 — 물리적 노출 메커니즘과 일치합니다.",
    paradoxStat1: "LOOCV RMSE",
    paradoxVal1: "0.522",
    paradoxStat2: "R²",
    paradoxVal2: "0.851",
    paradoxStat3: "0.5 이내 국가",
    paradoxVal3: "74%",
    paradoxCta: "횡단적 공식",
    ...
    epistemicNote: (n: number) => `BERM v17은 반증 가능한 연구 모델이며 확실성이 아닙니다. 여러 연구 분야에 걸친 ${n}개의 등록 출처. 24개 이상의 규제 검증된 비열적 메커니즘. 횡단적 공식: LOOCV RMSE 0.522, 74%의 국가가 예측에서 0.5명 이내. 주거용 전력 소비가 GDP를 TFR 예측 인자로 21% 능가합니다. 날짜와 신뢰구간이 있는 고정 예측. 예측이 실패하면, 모델은 틀립니다.`,
