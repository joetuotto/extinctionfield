...
const COPY = {
  en: {
    ...
    sSolarFormula: "BioCap(t,λ) = BioCap₀ − ∫₀ᵗ χ_lat(λ)·[S(τ) + U(τ) + E(τ)] dτ + ∫₀ᵗ α·χ_lat(λ)·[1−S(τ)]·[1−σ(τ)] dτ",
    sSolarFormulaTerms: [
    ...
    sThreeLaws: [
      { id: "H1", title: "Civilizational birth requires a low-χ_lat zone (25-35°N)", desc: "Biological stability → long-term development. The four primary civilizations (Mesopotamia, Egypt, Indus, Yellow River) arose at 25–35°N, where the separate BioCap factor χ_lat ≈ 0.37–0.49; the model's nine expansion empires lie at 33–52°N." },
      { id: "H2", title: "Creative renaissances cluster during grand solar minima at high-χ_lat latitudes (45-60°N)", desc: "Maximum recovery modulation → maximum creative capital (χ_lat ≈ 0.63–0.90 at 45–60°N). 8 of the 10 renaissances in the model dataset." },
      { id: "H3", title: "Empire rises begin during low solar activity", desc: "Biological recovery → capacity for expansion. Model dataset: mean solar index 0.41 at empire rises vs 0.49 at peaks." },
    ],
    ...
    sSolarFormulaTerms: [
      ...
      "χ_lat(λ) = erillinen BioCapin leveysasteherkkyys, joka kasvaa 0,25:stä päiväntasaajalla arvoon 1,0 65°:ssa ja sisältää alueen sähköistyslisän (chi_total); se ei ole Lindgrenin χ(Ā)",
    ...
    sThreeLaws: [
      { id: "H1", title: "Sivilisaation synty vaatii matalan χ_lat:n vyöhykettä (25-35°N)", desc: "Biologinen stabiilisuus → pitkäjänteinen kehitys. Neljä ensimmäistä sivilisaatiota (Mesopotamia, Egypti, Indus, Keltainenjoki) syntyivät 25–35°N:ssä, jossa erillinen BioCap-tekijä χ_lat ≈ 0,37–0,49; mallin yhdeksän ekspansioimperiumia sijaitsevat 33–52°N:ssä." },
      { id: "H2", title: "Luovat renessanssit klusteroituvat suurminimien aikaan korkean χ_lat:n leveysasteilla (45-60°N)", desc: "Suurin palautumismodulaatio → suurin luova pääoma (χ_lat ≈ 0,63–0,90 45–60°N:ssä). 8 mallin aineiston 10 renessanssista." },
      { id: "H3", title: "Imperiumien nousut alkavat matalan aurinkoaktiivisuuden aikana", desc: "Biologinen palautuminen → kapasiteetti ekspansioon. Mallin aineisto: aurinkoindeksin keskiarvo 0,41 imperiumien nousuissa vs 0,49 huipuissa." },
    ],
  ...
  ja: {
    ...
    sThreeLaws: [
      { id: "H1", title: "文明の誕生には低χ_lat帯（25-35°N）が必要", desc: "生物学的安定性→長期的発展。確認済み：p = 0.01（誕生地域対拡大）。" },
      { id: "H2", title: "創造的ルネサンスは高χ_lat緯度（45-60°N）の大極小期に集中する", desc: "最大回復変調→最大創造資本。確認済み：ヨーロッパの10件中8件のルネサンス。" },
      { id: "H3", title: "帝国の興隆は太陽活動が低い時期に始まる", desc: "生物学的回復→拡張のための能力。確認済み：興隆イベントのsolar = 0.41 対 ピークイベント = 0.73。" },
    ],
  ...
  fr: {
    ...
    sSolarFormula: "BioCap(t,λ) = BioCap₀ − ∫₀ᵗ χ_lat(λ)·[S(τ) + U(τ) + E(τ)] dτ + ∫₀ᵗ α·χ_lat(λ)·[1−S(τ)]·[1−σ(τ)] dτ",
    sSolarFormulaTerms: ["S(τ) = superposition des cycles solaires [0,1]", "U(τ) = composante EMF de l'urbanisation (lente, pre-electrique)", "E(τ) = composante EMF de l'electrification (rapide, post-1880)", "σ(τ) = coefficient de suppression de la recuperation = min(E(τ)/E_max, 0.95)", "χ_lat(λ) = facteur BioCap geomagnetique distinct (dependant de la latitude), different de χ(Ā) de Lindgren", "α = coefficient de recuperation biologique"],
    sSolarFormulaNote: "",
    ...
    sThreeLaws: [
      { id: "H1", title: "La naissance civilisationnelle requiert une zone a faible χ_lat (25-35°N)", desc: "Stabilite biologique → developpement a long terme. Confirme : p = 0.01 (regions de naissance vs expansions)." },
      { id: "H2", title: "Les renaissances creatives se regroupent pendant les grands minima solaires aux latitudes a χ_lat eleve (45-60°N)", desc: "Modulation de recuperation maximale → capital creatif maximal. Confirme : 8/10 renaissances europeennes." },
      { id: "H3", title: "Les ascensions imperiales commencent pendant les periodes de faible activite solaire", desc: "Recuperation biologique → capacite d'expansion. Confirme : evenements d'ascension solar = 0.41 vs evenements de pic = 0.73." },
    ],
  ...
  ko: {
    ...
    sSolarFormula: "BioCap(t,λ) = BioCap₀ − ∫₀ᵗ χ_lat(λ)·[S(τ) + U(τ) + E(τ)] dτ + ∫₀ᵗ α·χ_lat(λ)·[1−S(τ)]·[1−σ(τ)] dτ",
    sSolarFormulaTerms: ["S(τ) = 태양 주기 중첩 [0,1]", "U(τ) = 도시화의 EMF 성분 (느림, 전기 이전)", "E(τ) = 전기화의 EMF 성분 (빠름, 1880년 이후)", "σ(τ) = 회복 억제 계수 = min(E(τ)/E_max, 0.95)", "χ_lat(λ) = Lindgren의 χ(Ā)와 구별되는 BioCap 지자기 위도 계수", "α = 생물학적 회복 계수"],
    sSolarFormulaNote: "",
    ...
    sThreeLaws: [
      { id: "H1", title: "문명의 탄생은 낮은 χ_lat 지대(25-35°N)를 필요로 한다", desc: "생물학적 안정성 → 장기적 발전. 확인됨: p = 0.01 (탄생 지역 대 확장)." },
      { id: "H2", title: "창조적 르네상스는 높은 χ_lat 위도(45-60°N)의 대극소기에 집중된다", desc: "최대 회복 변조 → 최대 창조적 자본. 확인됨: 유럽 르네상스 10건 중 8건." },
      { id: "H3", title: "제국의 흥기는 낮은 태양 활동 기간에 시작된다", desc: "생물학적 회복 → 확장 능력. 확인됨: 흥기 이벤트 solar = 0.41 대 피크 이벤트 = 0.73." },
    ],
